...
export interface DiscoveredTalkTest {
  name: string;
  range: Range;
}
...
type ItemMetadata = FileMetadata | TestMetadata;
...
interface FinishedReport {
  status: "finished";
  failures: number;
  tests: TalkTestResult[];
  output: string;
}

interface NoTestsReport {
  status: "no_tests";
  failures: number;
  tests: [];
  output: string;
...
type TalkTestReport = FinishedReport | NoTestsReport | ErrorReport;
type InvocationStatus = "passed" | "failed" | "errored" | "cancelled" | "skipped";
...
function readTestName(line: string): string | undefined {
  if (/^\s*\/\//.test(line)) {
    return undefined;
  }
...
function findTestEnd(lines: string[], startLine: number): { line: number; character: number } {
  let depth = 0;
...
export function discoverTalkTests(source: string): DiscoveredTalkTest[] {
  const lines = source.split(/\r?\n/);
  const discovered: DiscoveredTalkTest[] = [];
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex += 1) {
    ...
    discovered.push({
      name,
      range: new Range(lineIndex, 0, end.line, end.character),
    });
  }
...
function isTestFile(uri: Uri): boolean {
  return uri.scheme === "file" && uri.path.endsWith(".test.tlk");
}

function isIgnored(uri: Uri): boolean {
  return uri.path.split("/").some((part) => ignoredDirectoryNames.has(part));
}

function reportFromJson(output: string): TalkTestReport | undefined {
  let parsed: unknown;
  try {
  ...
  if (value.status === "no_tests") {
    return value.tests.length === 0 ? (parsed as NoTestsReport) : undefined;
  }
  ...
  return parsed as FinishedReport;
}

function outputForTestRun(output: string): string {
  const normalized = output.replace(/\r?\n/g, "\r\n");
  return normalized.endsWith("\r\n") ? normalized : `${normalized}\r\n`;
}

function mergeInvocationStatus(
  current: InvocationStatus,
  next: InvocationStatus
): InvocationStatus {
  const priority: Record<InvocationStatus, number> = {
    skipped: 0,
    passed: 1,
    cancelled: 2,
    failed: 3,
...
function testMessage(message: string, item: TestItem): TestMessage {
  const result = new TestMessage(message);
  if (item.uri && item.range) {
    result.location = new Location(item.uri, item.range);
  }
  return result;
}
...
function collectionItems(item: TestItem): TestItem[] {
  const items: TestItem[] = [];
  item.children.forEach((child) => items.push(child));
  return items;
}

function appendReportOutput(run: TestRun, report: TalkTestReport, stderr: string, item?: TestItem) {
  if (report.output) {
...
export function registerTalkTestController(
  context: ExtensionContext,
  configuration: TestConfiguration
): TestController {
  const controller = tests.createTestController("talktalkTests", "TalkTalk");
  const metadata = new WeakMap<TestItem, ItemMetadata>();

  const updateFileFromSource = (uri: Uri, source: string) => {
    ...
    if (!file) {
      file = controller.createTestItem(id, workspace.asRelativePath(uri, false), uri);
      controller.items.add(file);
    }
    file.label = workspace.asRelativePath(uri, false);
  ...
  const updateFile = async (uri: Uri) => {
  ...
  const refresh = async (token?: CancellationToken) => {
    const files = await workspace.findFiles(testFilePattern, testFileExcludePattern);
  ...
  const buildRunPlans = (request: TestRunRequest): FileRunPlan[] => {
  ...
  const executeInvocation = async (
    ...
    const folder = workspace.getWorkspaceFolder(fileData.uri);
    const cwd = folder?.uri.fsPath ?? dirname(fileData.uri.fsPath);
    ...
    const resultsByName = new Map<string, TalkTestResult[]>();
    ...
    for (const item of selectedTests) {
      ...
      if (!result) {
        run.errored(
          item,
          testMessage(`talk test did not report a result for '${itemData.name}'`, item)
        );
        failed = true;
  ...
  const runHandler = async (request: TestRunRequest, token: CancellationToken) => {
    const run = controller.createTestRun(request);
    const plans = buildRunPlans(request);
    const unfinished = new Set<TestItem>();
    ...
    try {
      for (const plan of plans) {
        ...
        const document = workspace.textDocuments.find(
          (candidate) => candidate.uri.toString() === fileData.uri.toString()
        );
        if (document?.isDirty && !(await document.save())) {
          ...
          if (plan.includeFile) {
            run.errored(plan.file, new TestMessage(`Could not save ${fileData.uri.fsPath}`));
          }
          continue;
        }
        ...
        if (plan.tests.length === 0) {
          if (plan.includeFile) {
            ...
            if (result === "errored") {
              run.errored(plan.file, new TestMessage("talk test failed"));
            } else {
        ...
        let fileResult: InvocationStatus = "skipped";
        const allTests = collectionItems(plan.file);
        const runsWholeFile =
          plan.tests.length === allTests.length && allTests.every((item) => plan.tests.includes(item));
        ...
        if (runsWholeFile) {
          ...
          const testsByName = new Map<string, TestItem[]>();
          for (const item of plan.tests) {
            const itemData = metadata.get(item);
            if (!itemData || itemData.kind !== "test") {
              continue;
        ...
        if (plan.includeFile) {
          if (fileResult === "passed") {
            ...
            run.failed(plan.file, new TestMessage("One or more tests failed"));
