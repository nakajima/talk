...
pub enum Insn {
    ...
    RecordNew { dest: u16, symbol: symbol::Symbol, args_start: u32, args_len: u16 },
    GetField { dest: u16, rec: u16, index: u16 },
    VariantNew { dest: u16, symbol: symbol::Symbol, tag: u16, args_start: u32, args_len: u16 },
    GetTag { dest: u16, src: u16 },
    GetPayload { dest: u16, src: u16, index: u16 },
    ExistentialPack { dest: u16, protocol: symbol::Symbol, args_start: u32, args_len: u16 },
    ExistentialWitness { dest: u16, src: u16, index: u16 },
    ExistentialPayload { dest: u16, src: u16 },
    TupleNew { dest: u16, args_start: u32, args_len: u16 },
    Extract { dest: u16, src: u16, index: u16 },
    SetField { dest: u16, rec: u16, src: u16, index: u16 },
    Alloc { dest: u16, count: u16 },
    Free { dest: u16, ptr: u16 },
    ...
    Call { dest: u16, chunk: u32, args_start: u32, args_len: u16 },
    MakeClosure { dest: u16, chunk: u32, args_start: u32, args_len: u16 },
    EnvGet { dest: u16, index: u16 },
    CallIndirect { dest: u16, callee: u16, args_start: u32, args_len: u16 },
    Jump { target: u32 },
    Branch { cond: u16, then_target: u32, else_target: u32 },
    Switch { tag: u16, targets_start: u32, targets_len: u16 },
    Ret { src: u16 },
    Trap { message: u32 },
}
...
impl Module {
    ...
    fn render_insn(&self, insn: &Insn) -> String {
        match insn {
            ...
            Insn::RecordNew { dest, symbol, args_start, args_len } => format!("record_new r{dest} <- {symbol}({})", self.render_args(*args_start, *args_len)),
            Insn::GetField { dest, rec, index } => format!("get_field r{dest} <- r{rec}.{index}"),
            Insn::VariantNew { dest, symbol, tag, args_start, args_len } => format!("variant_new r{dest} <- {symbol}#{tag}({})", self.render_args(*args_start, *args_len)),
            Insn::GetTag { dest, src } => format!("get_tag r{dest} <- r{src}"),
            Insn::GetPayload { dest, src, index } => format!("get_payload r{dest} <- r{src}.{index}"),
            Insn::ExistentialPack { dest, protocol, args_start, args_len } => format!("existential_pack r{dest} <- any {protocol}({})", self.render_args(*args_start, *args_len)),
            Insn::ExistentialWitness { dest, src, index } => format!("existential_witness r{dest} <- r{src}.{index}"),
            Insn::ExistentialPayload { dest, src } => format!("existential_payload r{dest} <- r{src}"),
            Insn::TupleNew { dest, args_start, args_len } => format!("tuple r{dest} <- ({})", self.render_args(*args_start, *args_len)),
            Insn::Extract { dest, src, index } => format!("extract r{dest} <- r{src}.{index}"),
            Insn::SetField { dest, rec, src, index } => format!("set_field r{dest} <- r{rec} with .{index} = r{src}"),
            Insn::Alloc { dest, count } => format!("alloc r{dest} <- r{count} bytes"),
            Insn::Free { dest, ptr } => format!("free r{dest} <- r{ptr}"),
            Insn::Load { dest, ptr, kind } => format!("load_{} r{dest} <- [r{ptr}]", format!("{kind:?}").to_lowercase()),
            Insn::Store { ptr, src, kind } => format!("store_{} [r{ptr}] <- r{src}", format!("{kind:?}").to_lowercase()),
            Insn::Copy { from, to, len } => format!("copy [r{to}] <- [r{from}], r{len} bytes"),
            Insn::Io { dest, op, a, b, c } => format!("io_{} r{dest} <- r{a}, r{b}, r{c}", format!("{op:?}").to_lowercase()),
            Insn::Call { dest, chunk, args_start, args_len } => format!("call r{dest} <- {}({})", self.chunks[*chunk as usize].name, self.render_args(*args_start, *args_len)),
            Insn::MakeClosure { dest, chunk, args_start, args_len } => format!("closure r{dest} <- {} capturing ({})", self.chunks[*chunk as usize].name, self.render_args(*args_start, *args_len)),
            Insn::EnvGet { dest, index } => format!("env_get r{dest} <- env[{index}]"),
            Insn::CallIndirect { dest, callee, args_start, args_len } => format!("call_indirect r{dest} <- r{callee}({})", self.render_args(*args_start, *args_len)),
            Insn::Jump { target } => format!("jump {target}"),
            Insn::Branch { cond, then_target, else_target } => format!("branch r{cond} ? {then_target} : {else_target}"),
            Insn::Switch { tag, targets_start, targets_len } => {
                let targets = &self.switch_pool[*targets_start as usize..*targets_start as usize + *targets_len as usize];
...
impl Styles {
    pub fn plain() -> Self {
        Self { func: "", keyword: "", reset: "" }
    }

    pub fn ansi() -> Self {
        Self { func: "\x1b[1;33m", keyword: "\x1b[1;35m", reset: "\x1b[0m" }
    }
}
