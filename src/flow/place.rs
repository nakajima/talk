...
pub fn place_for_expr(expr: &crate::typed_ast::Expr) -> Option<Place> {
    match &expr.kind {
        crate::typed_ast::ExprKind::Variable(name) => name.symbol().ok().map(Place::root),
        crate::typed_ast::ExprKind::Proj(receiver, _, field) => {
