...
mod with_core {
    ...
    #[test]
    fn conditional_cheap_clone_context_gates_borrow_extraction() {
        // The conditional row's where-clause is the authority for flow's
        // cheap-clone judgment too: extracting Box<NotCheap> from a borrow
        // must not silently clone through a row whose context fails.
        let t = check_with_core(Source::from(
            "struct Box<T> {\n\tlet value: T\n}\nextend Box<T>: CheapClone where T: CheapClone {}\nstruct NotCheap {\n\tlet value: String\n}\nfunc peek(b: &Box<NotCheap>?) -> Box<NotCheap>? {\n\tmatch b {\n\t\t.some(found) -> Optional.some(found),\n\t\t.none -> Optional.none\n\t}\n}",
        ));
        assert!(
            t.has_errors(),
            "expected flow to reject the borrowed extraction; program passed clean"
        );
    }
