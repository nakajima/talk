...
use crate::{
    interpret::{arena::Arena, interpreter_v2::InterpreterError},
...
impl Memory {
    ...
    pub fn store(&mut self, pointer: &Pointer, value: Value) -> Result<(), InterpreterError> {
        ...
        match pointer.location {
            MemoryLocation::Stack => {
                ...
                Self::store_value_to_slice(
                    &mut self.stack[pointer.addr..(pointer.addr + info.size)],
                    value,
                    &info,
                )?;
            }
            MemoryLocation::Heap => unsafe {
            ...
            MemoryLocation::Static => {
                return Err(InterpreterError::Unknown(
                    "Cannot write to static memory".into(),
                ));
            }
        }
        ...
        match pointer.location {
            ...
            MemoryLocation::Static => {
                if pointer.addr + info.size > self.static_data.len() {
                    return Err(InterpreterError::Unknown(
                        "Static memory out of bounds".into(),
                    ));
                }
    ...
    fn store_value_to_slice(
        ...
        match (&value, &info.ty) {
            ...
            (Value::Immediate(i), IRType::Pointer { .. }) => {
                // Handle pointers stored as immediate values (e.g., static pointers)
                slice[..8].copy_from_slice(&i.to_ne_bytes());
            }
            _ => {
                return Err(InterpreterError::Unknown(format!(
                    "Type mismatch in store: {:?} vs {:?}",
                    value, info.ty
                )));
            }
        }
    ...
    fn load_value_from(&self, slice: &[u8], info: &TypeInfo) -> Result<Value, InterpreterError> {
        match &info.ty {
            ...
            IRType::Bool => Ok(Value::Bool(slice[0] != 0)),
            IRType::Byte => Ok(Value::Immediate(slice[0] as i64)),
            IRType::Pointer { .. } => {
                let bytes = slice[..8]
                    .try_into()
                    .map_err(|_| InterpreterError::Unknown("Invalid load".into()))?;
