...
impl SemanticIndex {
    ...
    pub fn debug_member_accesses(&self) -> Vec<(&ExprID, &str, Option<SymbolID>)> {
        ...
        for (expr_id, expr) in &self.expressions {
            if let ResolvedExpr::MemberAccess { member_name, resolved_symbol, .. } = expr {
    ...
    pub fn record_expr_span(&mut self, expr_id: ExprID, span: Span) {
        self.expr_spans.insert(expr_id, span);
    }
    ...
    pub fn find_expr_by_span(&self, span: Span) -> Option<ExprID> {
        ...
        self.expr_spans.iter()
            .find(|(_, s)| **s == span)
            .map(|(id, _)| *id)
    }
    
    /// Find expression containing a position
    pub fn find_expr_at_position(&self, position: &crate::diagnostic::Position, path: &std::path::PathBuf) -> Option<ExprID> {
        // Find the smallest span that contains the position
        let mut smallest_span: Option<(&ExprID, &Span)> = None;
        let mut smallest_length = u32::MAX;
        
        for (expr_id, span) in &self.expr_spans {
            if span.path == *path && span.contains(&position) {
                let length = span.length();
                if length < smallest_length {
                    smallest_length = length;
                    smallest_span = Some((expr_id, span));
                }
            }
        }
        
        smallest_span.map(|(id, _)| *id)
    }
}
...
impl QueryDatabase for SemanticIndex {
    fn expr_type(&self, expr: ExprID) -> Option<Ty> {
        self.expressions.get(&expr).map(|resolved| {
            match resolved {
    ...
    fn expr_symbol(&self, expr: ExprID) -> Option<SymbolID> {
        self.expressions.get(&expr).and_then(|resolved| {
            match resolved {
                ResolvedExpr::Variable { symbol, .. } => Some(*symbol),
                ResolvedExpr::MemberAccess { resolved_symbol, .. } => *resolved_symbol,
                _ => None,
            }
        })
    }
    ...
    fn type_members(&self, type_symbol: SymbolID) -> Option<&[MemberInfo]> {
        self.type_info.get(&type_symbol).map(|info| &info.members[..])
    }
