...
impl TypeChecker {
    ...
    pub fn infer_node(
        ...
        let ty = match &expr {
            Expr::LiteralTrue | Expr::LiteralFalse => self.infer_literal_bool(env),
            Expr::Loop(_, _) => self.infer_loop(env),
            Expr::If(_, _, _) => self.infer_if(env),
            ...
            Expr::Block(items) => self.infer_block(env, items, expected, source_file),
            ...
            Expr::Member(receiver, member_name) => {
                self.infer_member(env, receiver, member_name, source_file)
            }
    ...
    fn infer_literal_bool(&self, _env: &mut Environment) -> Result<Ty, TypeError> {
        todo!()
    }

    fn infer_loop(&self, _env: &mut Environment) -> Result<Ty, TypeError> {
        todo!()
    }

    fn infer_if(&self, _env: &mut Environment) -> Result<Ty, TypeError> {
        todo!()
    }
    ...
    fn infer_call(
        ...
        env.constraints
            .push(Constraint::Equality(expected_callee_ty, callee_ty.clone()));
    ...
    fn infer_assignment(
        ...
        env.constraints
            .push(Constraint::Equality(lhs_ty.clone(), rhs_ty));
    ...
    fn infer_func(
        ...
        let ret_type_expr: Option<Ty> = ret
            .as_ref()
        ...
        if let Some(ret_type) = ret_type_expr {
            env.constraints
                .push(Constraint::Equality(body_ty.clone(), ret_type));
        }
        ...
        if let Some(func_var) = func_var {
            env.constraints
                .push(Constraint::Equality(Ty::TypeVar(func_var), func_ty.clone()));
    ...
    fn infer_block(
        ...
        let return_ty = if let Some(expected) = expected {
            if return_ty != *expected {
                env.constraints
                    .push(Constraint::Equality(return_ty, expected.clone()));
            }
    ...
    fn infer_match(
        ...
        let items_ty = items_ty.iter().map(|ty| ty.clone()).collect::<Vec<_>>();
    ...
    fn infer_match_arm(
        ...
        env.start_scope();
        let pattern_ty = self.infer_node(pattern, env, expected, source_file)?;
    ...
    fn infer_member(
        ...
        match receiver {
            ...
            Some(receiver_id) => {
                ...
                let instantiated_receiver_ty = match &receiver_ty {
