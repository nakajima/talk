...
pub enum MissingPattern {
    ...
    Variants { enum_name: String, variant_names: Vec<String> },
...
impl<'a> ExhaustivenessChecker<'a> {
    ...
    pub fn check_match(
        ...
        match scrutinee_ty {
            Ty::Enum(enum_id, _type_args) => {
                self.check_enum_exhaustiveness(enum_id, patterns)
            }
            Ty::TypeVar(type_var) => {
                self.check_type_var_exhaustiveness(type_var, patterns)
            }
            Ty::Bool => {
    ...
    fn check_enum_exhaustiveness(
        ...
        let all_variants: BTreeSet<_> = enum_def
            .variants()
            .iter()
            .map(|v| v.name.clone())
            .collect();
