...
/// The RawFormatter doesn't require expr metadata, but it might not be as accurate
...
impl RawFormatter {
    ...
    pub fn format_single_expr(parsed_expr: &Expr) -> String {
    ...
    pub fn format(&self, roots: &[ParsedExpr], width: usize) -> String {
        ...
        for (i, root) in roots.iter().enumerate() {
            ...
            let doc = self.format_expr(&root.expr);
    ...
    pub(crate) fn format_expr(&self, expr: &Expr) -> Doc {
        match &expr {
            ...
            Expr::Import(name) => join(vec![text("import"), text(name)], text(" ")),
            Expr::Call {
                callee,
                type_args,
                args,
            ...
            Expr::Member(receiver, property) => self.format_member(receiver, &property.to_string()),
            Expr::Func {
                name,
                generics,
                params,
            ...
            Expr::Spread(expr) => join(vec![text("..."), self.format_expr(&expr.expr)], text("")),
        }
    }

    fn format_arg(&self, label: &Option<Name>, value: &ParsedExpr) -> Doc {
        let Some(name) = label else {
            return self.format_expr(&value.expr);
        };

        group(concat(
            concat(self.format_name(name), text(": ")),
            self.format_expr(&value.expr),
        ))
    }
    ...
    fn format_record_literal(&self, fields: &[ParsedExpr]) -> Doc {
        ...
        let formatted_fields = fields
            .iter()
            .map(|field| self.format_expr(&field.expr))
            .collect::<Vec<_>>();
    ...
    fn format_record_type(&self, fields: &[ParsedExpr], row_var: &Option<Box<ParsedExpr>>) -> Doc {
        ...
        for field in fields {
            all_elements.push(self.format_expr(&field.expr));
        }

        if let Some(row) = row_var {
            all_elements.push(self.format_expr(&row.expr));
        }
    ...
    fn format_array_literal(&self, items: &[ParsedExpr]) -> Doc {
        ...
        let elements = items.iter().map(|id| self.format_expr(&id.expr)).collect();
    ...
    fn format_unary(&self, op: &TokenKind, rhs: &ParsedExpr) -> Doc {
        ...
        concat(text(op_text), self.format_expr(&rhs.expr))
    }
    ...
    fn format_binary(&self, lhs: &ParsedExpr, op: &TokenKind, rhs: &ParsedExpr) -> Doc {
        ...
        group(concat_space(
            self.format_expr(&lhs.expr),
            concat_space(text(op_text), self.format_expr(&rhs.expr)),
        ))
    }
    ...
    fn format_tuple(&self, items: &[ParsedExpr]) -> Doc {
        ...
        if items.len() == 1 {
            return concat(
                text("("),
                concat(self.format_expr(&items[0].expr), text(")")),
            );
        }
    ...
    fn format_block(&self, stmts: &[ParsedExpr]) -> Doc {
        ...
        if stmts.len() == 1 && !Self::contains_control_flow(&stmts[0]) {
            return group(concat(
                text("{"),
                concat(
                    concat(text(" "), self.format_expr(&stmts[0].expr)),
                    text(" }"),
                ),
            ));
        }
        ...
        for (i, stmt) in stmts.iter().enumerate() {
            ...
            final_doc = concat(final_doc, self.format_expr(&stmt.expr));
        }
    ...
    fn format_call(
        ...
        let mut result = self.format_expr(&callee.expr);

        if !type_args.is_empty() {
            let type_docs: Vec<_> = type_args
                .iter()
                .map(|id| self.format_expr(&id.expr))
                .collect();
        ...
        let arg_docs: Vec<_> = args.iter().map(|id| self.format_expr(&id.expr)).collect();
        ...
        match pattern {
            ...
            Pattern::Variant {
                ...
                if !fields.is_empty() {
                    let field_docs: Vec<_> =
                        fields.iter().map(|id| self.format_expr(&id.expr)).collect();
            ...
            Pattern::Struct {
                ...
                for (field_name, field_pattern) in field_names.iter().zip(fields.iter()) {
                    ...
                    let is_shorthand = if let Expr::ParsedPattern(Pattern::Bind(bind_name)) =
                        &field_pattern.expr
                    {
                    ...
                    if !is_shorthand {
                        field_doc = concat(
                            field_doc,
                            concat(text(": "), self.format_expr(&field_pattern.expr)),
                        );
                    }
    ...
    fn format_return(&self, value: &Option<Box<ParsedExpr>>) -> Doc {
        match value {
            Some(id) => concat_space(text("return"), self.format_expr(&id.expr)),
            None => text("return"),
        }
    }
    ...
    fn format_struct(
        ...
        concat_space(result, self.format_expr(&body.expr))
    }
    ...
    fn format_property(
        ...
        if let Some(type_id) = type_repr {
            result = concat(
                result,
                concat_space(text(":"), self.format_expr(&type_id.expr)),
            );
        }
        ...
        if let Some(value_id) = default_value {
            result = concat_space(
                result,
                concat_space(text("="), self.format_expr(&value_id.expr)),
            );
        }
    ...
    fn format_func_type_repr(&self, args: &[ParsedExpr], ret: &ParsedExpr) -> Doc {
        ...
        concat(
            ...
            concat(
                ...
                concat_space(text(") ->"), self.format_expr(&ret.expr)),
            ),
        )
    }

    fn format_tuple_type_repr(&self, types: &[ParsedExpr]) -> Doc {
        let type_docs: Vec<_> = types.iter().map(|id| self.format_expr(&id.expr)).collect();
    ...
    fn format_func(
        ...
        let param_docs: Vec<_> = params.iter().map(|id| self.format_expr(&id.expr)).collect();
        ...
        if let Some(ret_id) = ret {
            result = concat_space(
                result,
                concat_space(text("->"), self.format_expr(&ret_id.expr)),
            );
        }
        ...
        if let Some(body) = body {
            ...
            {
                return group(concat_space(result, self.format_expr(&body.expr)));
            }
    ...
    fn format_assignment(&self, lhs: &ParsedExpr, rhs: &ParsedExpr) -> Doc {
        concat_space(
            self.format_expr(&lhs.expr),
            concat_space(text("="), self.format_expr(&rhs.expr)),
        )
    }
    ...
    fn format_if(
        ...
        let mut result = concat_space(
            text("if"),
            concat_space(
                self.format_expr(&cond.expr),
                self.format_expr(&then_block.expr),
            ),
        );
        ...
        if let Some(else_id) = else_block {
            result = concat_space(
                result,
                concat_space(text("else"), self.format_expr(&else_id.expr)),
            );
        }
    ...
    fn format_loop(&self, cond: &Option<Box<ParsedExpr>>, body: &ParsedExpr) -> Doc {
        ...
        if let Some(cond_id) = cond {
            result = concat_space(result, self.format_expr(&cond_id.expr));
        }
    ...
    fn format_enum_body(&self, body_id: &ParsedExpr) -> Doc {
        ...
        for item_id in items {
            docs.push(self.format_expr(&item_id.expr));
        }
    ...
    fn format_match(&self, target: &ParsedExpr, arms: &[ParsedExpr]) -> Doc {
        let arms_docs: Vec<_> = arms.iter().map(|id| self.format_expr(&id.expr)).collect();

        concat_space(
            text("match"),
            concat_space(
                self.format_expr(&target.expr),
                concat(
                    text("{"),
                    concat(
    ...
    fn format_match_arm(&self, pattern: &ParsedExpr, body: &ParsedExpr) -> Doc {
        concat_space(
            self.format_expr(&pattern.expr),
            concat_space(text("->"), self.format_expr(&body.expr)),
        )
    }
    ...
    fn format_pattern_variant(
        ...
        if !bindings.is_empty() {
            let binding_docs: Vec<_> = bindings
                .iter()
                .map(|id| self.format_expr(&id.expr))
                .collect();
