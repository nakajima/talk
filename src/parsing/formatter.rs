...
impl<'a> Formatter<'a> {
    ...
    pub(crate) fn format_expr(&self, expr: &ParsedExpr) -> Doc {
        match &expr.expr {
            ...
            Expr::Parameter(name, type_repr, _) => self.format_parameter(name, type_repr),
            Expr::Let(name, type_repr, _) => self.format_let(name, type_repr),
            ...
            Pattern::Struct {
                ...
                for (field_name, field_pattern) in field_names.iter().zip(fields.iter()) {
                    ...
                    if !is_shorthand {
                        field_doc = concat(
                            field_doc,
                            concat(text(": "), self.format_expr(field_pattern)),
                        );
                    }
