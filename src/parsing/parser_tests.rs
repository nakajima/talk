...
pub mod tests {
    ...
    #[test]
    fn rejects_out_of_range_inline_ir_integer_without_panicking() {
        let lexer = Lexer::new("@_ir { %? = add Int 9_223_372_036_854_775_808 %0 }");
        let parser = Parser::new("-", FileID(0), lexer);
        assert!(matches!(
            parser.parse(),
            Err(ParserError::IntegerLiteralOutOfRange { .. })
        ));
    }
