...
impl Scheme {
    /// No quantified parameters of any kind: instantiation would be the
    /// identity, so references use the type directly.
    pub fn is_monomorphic(&self) -> bool {
        self.params.is_empty()
            && self.eff_params.is_empty()
            && self.row_params.is_empty()
            && self.perm_params.is_empty()
    }
...
/// One kind's binder correspondence: left binders map to right binders
/// injectively (the reverse map rejects two lefts claiming one right);
/// symbols outside the correspondence must match themselves.
#[derive(Clone, Default)]
struct Binders {
    forward: FxHashMap<Symbol, Symbol>,
    reverse: FxHashMap<Symbol, Symbol>,
}

impl Binders {
    fn bind(&mut self, left: Symbol, right: Symbol) {
        self.forward.insert(left, right);
        self.reverse.insert(right, left);
    }

    fn eq(&self, left: Symbol, right: Symbol) -> bool {
        match (self.forward.get(&left), self.reverse.get(&right)) {
...
impl AlphaEq {
    ...
    fn perms(&self, left: Perm, right: Perm) -> bool {
        match (left, right) {
            (Perm::Param(left), Perm::Param(right)) => self.perms.eq(left, right),
    ...
    fn tys(&self, left: &Ty, right: &Ty) -> bool {
        match (left, right) {
            ...
            (Ty::Param(left), Ty::Param(right)) => self.tys.eq(*left, *right),
