...
impl TypeCatalog {
    ...
    /// Normalize a finished (variable-free) type against this catalog:
    /// alias expansion and projection reduction, no unification state.
    /// The catalog-level API lowering uses; solver internals stay in
    /// `types::solve` (ADR 0057).
    pub fn normalize_finished(&self, ty: &Ty) -> Ty {
        let mut scratch = crate::types::solve::VarStore::default();
        crate::types::solve::normalize_ty(&mut scratch, self, ty)
    }
    ...
    #[allow(clippy::disallowed_methods)] // self-recursion over components
