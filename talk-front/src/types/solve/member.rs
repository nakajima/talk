...
impl<'s> Solver<'s> {
        ...
        match member_receiver.clone() {
            ...
            Ty::Nominal(symbol, args) => {
                ...
                if let Some(members) = self.catalog.extend_members.get(&symbol)
                    && members.get(&label_str).is_some_and(|rows| {
                        rows.iter().any(|row| {
                            self.catalog
                                .callable_contracts
                                .get(&row.symbol)
                ...
                hidden |= self
                    ...
                    .is_some_and(|rows| {
                        let instance_rows = rows.iter().filter(|row| {
                            self.catalog
                                .callable_contracts
                                .get(&row.symbol)
                                .is_some_and(|contract| {
                        ...
                        let mut found = false;
                        let hidden = instance_rows.fold(true, |hidden, row| {
                            found = true;
                            hidden && !self.member_accessible(row.symbol, origin)
                        });
                        found && hidden
                    });
    ...
    /// Resolve a leading-dot expression against its contextual type. Enum
    /// cases and static functions are both type members; unlike HasVariant,
    /// this constraint is never generated by pattern checking.
    pub(super) fn try_type_member(
        &mut self,
        receiver: Ty,
        label: Label,
        ...
        let (lookup, _) = self.member_receivers(&receiver);
        if stuck_projection(self.store, &lookup) {
            return Some(Constraint::HasTypeMember {
                receiver,
                label,
                payload,
                ctor,
        ...
        let Ty::Nominal(symbol, args) = head else {
            return match head {
                Ty::Var(_) => Some(Constraint::HasTypeMember {
                    receiver,
                    label,
                    payload,
                    ctor,
        ...
        // Enum cases are static type members. Reuse the pattern-safe
        // constructor instantiation, but only expression generation can
        // enter this type-member path.
        if self
            .catalog
            .enums
            .get(&symbol)
            .is_some_and(|info| info.variants.contains_key(&label_str))
        {
            return self.try_variant(receiver, label, payload, ctor, origin, queue);
        }
        ...
        let static_members = self
            ...
            .structs
            .get(&symbol)
            .map(|info| (info.params.clone(), info.statics.get(&label_str).cloned()))
            .or_else(|| {
                self.catalog
                    .enums
                    .get(&symbol)
        ...
        if let Some((params, Some(set))) = static_members {
            let accessible: Vec<Symbol> = set
                .iter()
                .copied()
                .filter(|method| self.member_accessible(*method, origin))
            ...
            let member_origin = CtOrigin::new(member_node, origin.reason);
            let method = self.select_method_overload(&accessible, &label_str, member_origin)?;
            let substitution: FxHashMap<Symbol, Ty> = params
                .iter()
                .map(|param| param.symbol)
                .zip(args.iter().cloned())
                .collect();
            let signature = self.symbol_ty(method, member_node, queue).substitute(
                &substitution,
                &Default::default(),
                &Default::default(),
            );
            match (ctor.clone(), self.store.shallow(&signature)) {
                (Some(ctor), Ty::Func(params, ret, effects)) => {
                    if params.len() != payload.len() {
                        self.errors.push((
                            TypeError::ArgumentArityMismatch {
                                target: format!("Static member '{label}'"),
                                expected: params.len(),
                                found: payload.len(),
                            },
                            origin.node,
                        ...
                        for (expected, (_, found)) in params.iter().zip(&payload) {
                            self.push_immediate_argument_eq(
                                queue,
                                expected.clone(),
                                found.clone(),
                    ...
                    queue.push(Constraint::Eq(lookup, (*ret).clone(), origin));
                    if let Some(allowed) = allowed_effects {
                        queue.push(Constraint::EffectSubset {
                            inferred: effects.clone(),
                            allowed,
                            origin,
                        });
                    }
                    queue.push(Constraint::Eq(ctor, signature, origin));
                }
                (None, _) => queue.push(Constraint::Eq(lookup, signature, origin)),
                (Some(_), Ty::Var(_)) => {
                    return Some(Constraint::HasTypeMember {
                        receiver,
                        label,
                ...
                (Some(_), _) => {
                    self.errors.push((
                        TypeError::NotAFunction {
                            found: self.store.render(&signature),
                        },
                        origin.node,
                    ));
                }
            }
            self.instantiations.entry(member_node).or_default().extend(
                substitution
                    .iter()
                    .map(|(symbol, ty)| (*symbol, ty.clone())),
            );
            self.member_resolutions
                .insert(member_node, MemberResolution::Direct(method));
            return None;
        }

        // Inherent static extension members use the same disjoint instance
        // rows as instance extensions, filtered by their callable role. The
        // contextual nominal application selects the row.
        let extension_rows: Vec<_> = self
            .catalog
            .extend_members
            .get(&symbol)
            .and_then(|members| members.get(&label_str))
            ...
            .cloned()
            .collect();
        if !extension_rows.is_empty() && args.iter().any(Ty::has_unification_vars) {
            return Some(Constraint::HasTypeMember {
                receiver,
                label,
                payload,
        ...
        let accessible: Vec<_> = extension_rows
            .iter()
            .filter(|row| self.member_accessible(row.symbol, origin))
            .filter(|row| {
                let mut probe = FxHashMap::default();
        ...
        if let Some(inherent) = accessible.first() {
            let mut substitution = FxHashMap::default();
            for (pattern, actual) in inherent.self_args.iter().zip(&args) {
                bind_param_pattern(pattern, actual, &mut substitution);
            }
            let Some(mut scheme) = self.schemes.get(&inherent.symbol).cloned() else {
                return None;
            };
            scheme.ty =
                scheme
            ...
            scheme.predicates = scheme
                ...
                .collect();
            let signature = self.instantiate_scheme(&scheme, member_node, queue);
            match (ctor.clone(), self.store.shallow(&signature)) {
                (Some(ctor), Ty::Func(params, ret, effects)) => {
                    if params.len() != payload.len() {
                        self.errors.push((
            ...
            self.member_resolutions
                .insert(member_node, MemberResolution::Direct(inherent.symbol));
            return None;
        }
    ...
    fn push_immediate_argument_eq(
        ...
        match crate::types::adapt::adapt(
            self.store,
            self.catalog,
            &expected,
            &found,
            crate::types::adapt::Site::Argument,
        ) {
            Adapted::Eq {
                expected, found, ..
            }
