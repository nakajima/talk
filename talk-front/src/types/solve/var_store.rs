...
/// What a scheme reference does beyond freshening: a generate-time
/// reference to a binding mints static holes (with their nonnegativity
/// wanteds, ADR 0035 §2/§5) and takes declared defaults as `PreferEq`
/// fallbacks; solver-side instantiations (member dispatch, `Forall`
/// elimination) do neither.
...
impl VarStore {
    /// Instantiate a scheme with fresh variables (Damas-Milner): every
    /// quantified type/effect/row/perm parameter freshens at `level`, the
    /// qualified context is re-emitted through `out` under the
    /// instantiation substitution (Jones's qualified-type instantiation;
    /// perms substitute too, so a predicate naming a quantified
    /// `Perm::Param` sees the same fresh var the scheme type gets), and
    /// the substitution is returned for the lowerer's per-call-site θ —
    /// type params, plus row params as empty open records that finalize
    /// zonks to the call's concrete row. Callers own the early return
    /// for monomorphic schemes and the recording of θ.
    pub(crate) fn instantiate_scheme(
        ...
        for param in &scheme.params {
            let fresh = self.fresh_ty(level, origin.node);
            if site.statics
                && let crate::types::ty::ParamKind::Static(value_ty) = &param.kind
            {
                // ADR 0035 §5: a hole standing for a static argument left
                // unresolved at finalization gets a targeted diagnostic.
                self.mark_static_hole(fresh);
        ...
        for param in &scheme.eff_params {
            effs.insert(*param, EffTail::Var(self.fresh_eff(level, origin.node)));
        }
        let mut rows = FxHashMap::default();
        for param in &scheme.row_params {
            let var = self.fresh_row(level, origin.node);
            recorded.push((
                *param,
                Ty::Record(Row {
                    fields: vec![],
        ...
        for param in &scheme.perm_params {
            perms.insert(*param, Perm::Var(self.fresh_perm(level, origin.node)));
        }
        if site.defaults {
