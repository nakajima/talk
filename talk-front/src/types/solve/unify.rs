...
impl<'s> Solver<'s> {
    ...
    fn push_apply_param_eq(
        ...
        match adapt(self.store, self.catalog, expected, found, Site::Argument) {
            ...
            // sight commits to plain equality — that is the progress the
            // Eq worklist owes its callers (givens rewriting included).
            Adapted::Donate { .. }
            | Adapted::NoEvidence { .. }
            | Adapted::Unresolved {
                visible_borrow: true,
            ...
            // Eager peel: the in-group binding feeds recursion-group
            // generalization (see the variant's doc).
            Adapted::PeelableProjection {
                expected_inner,
                found,
            } => worklist.push(Constraint::Eq(expected_inner, found, origin)),
            Adapted::Silent => {}
        }
    }
