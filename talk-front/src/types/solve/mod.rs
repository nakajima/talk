...
impl<'s> Solver<'s> {
    ...
    fn solve_adapt(
        ...
        let site = if matches!(origin.reason, CtReason::Return | CtReason::Body) {
            Site::Return
        } else {
            Site::Argument
        };
        match adapt(self.store, self.catalog, &expected, &found, site) {
            Adapted::Eq {
                expected, found, ..
    ...
    fn instantiate_forall(&mut self, ty: Ty, origin: CtOrigin, queue: &mut Vec<Constraint>) -> Ty {
        ...
        let (ty, recorded) =
            self.store
                .instantiate_scheme(&scheme, self.level, origin, queue, SchemeUse::default());
        self.instantiations
            .entry(origin.node)
            .or_default()
...
pub(super) use pattern::bind_param_pattern;
pub(crate) use var_store::TyNode;
pub(crate) use var_store::SchemeUse;
