...
impl<'a> Higlighter<'a> {
    ...
    /// Highlight from the caller's lexed tokens and optional parse:
    /// the lex/parse-free half of highlighting. The orchestration that
    /// runs the self-hosted lexer/parser (`frontend::highlight`,
    /// `frontend::highlight_with_ast`) lives root-side (ADR 0057
    /// slice 3).
    pub fn highlight_from(
        &mut self,
        lexed: &[crate::token::Token],
        ast: Option<&AST<Parsed>>,
    ) -> Vec<HighlightToken> {
        let mut result = self.collect_lexed_tokens(lexed);
        if let Some(ast) = ast {
    ...
    fn collect_lexed_tokens(&mut self, lexed: &[crate::token::Token]) -> Vec<HighlightToken> {
        ...
        for (index, tok) in lexed.iter().enumerate() {
            match tok.kind {
                ...
                TokenKind::LessLess => self.make(tok, Kind::OPERATOR, &mut tokens),
                TokenKind::Greater => self.make(tok, Kind::OPERATOR, &mut tokens),
                TokenKind::GreaterEquals => self.make(tok, Kind::OPERATOR, &mut tokens),
                TokenKind::GreaterGreater => self.make(tok, Kind::OPERATOR, &mut tokens),
