...
pub mod tests {
    ...
    use talk_front::diagnostic::AnyDiagnostic;
    ...
    pub fn compile_library(
        name: &str,
        id: talk_front::front::module::ModuleId,
        source: &'static str,
        deps: talk_front::front::module::ModuleEnvironment,
    ) -> talk_front::front::module::Module {
        let mut config = DriverConfig::new(name);
        config.module_id = id;
        config.mode = talk::compiling::driver::CompilationMode::Library;
    ...
    /// Every expression node of the typed tree, cloned out for
    /// assertions on baked per-node facts (the tree is the one authority
    /// for these since ADR 0057; the checker publishes no NodeID tables).
    pub fn typed_exprs(driver: &Driver<Typed>) -> Vec<talk_front::typed_ast::Expr> {
        use derive_visitor::{Drive, Visitor};
        #[derive(Visitor)]
        #[visitor(talk_front::typed_ast::Expr(enter))]
        struct All {
            found: Vec<talk_front::typed_ast::Expr>,
        }
        impl All {
            fn enter_expr(&mut self, e: &talk_front::typed_ast::Expr) {
                self.found.push(e.clone());
            }
        }
        let mut all = All { found: vec![] };
    ...
    pub fn ty_of(driver: &Driver<Typed>, name: &str) -> String {
        ...
        let _names = talk_front::name_resolution::symbol::set_symbol_names(
    ...
    pub fn type_errors(driver: &Driver<Typed>) -> Vec<String> {
        types_with_severity(driver, talk_front::diagnostic::Severity::Error)
    }

    pub fn type_warnings(driver: &Driver<Typed>) -> Vec<String> {
        types_with_severity(driver, talk_front::diagnostic::Severity::Warn)
    }

    fn types_with_severity(
        driver: &Driver<Typed>,
        severity: talk_front::diagnostic::Severity,
    ) -> Vec<String> {
        driver
            .phase
            .diagnostics
    ...
    pub fn undisplayable_diagnostics(driver: &Driver<Typed>) -> Vec<String> {
        driver
            ...
            .filter_map(|d| {
                ...
                (id.0 == talk_front::node_id::FileID::SYNTHESIZED).then(|| d.to_string())
            })
            .collect()
    }
    ...
    fn expected_any_protocol_implicitly_packs_conforming_values() {
        ...
        let packs = typed_exprs(&t)
            .iter()
            .filter(|e| e.existential_pack.is_some())
            .count();
        assert_eq!(packs, 1);
    }
    ...
    fn implicit_pack_of_borrowed_clone_payload_records_a_clone() {
        ...
        .type_check();
        assert_clean(&t);
        // No explicit `clone` appears in the source, so a baked
    ...
    fn node_types_recorded_for_expressions() {
        let t = check("// no-core\nlet a = 123");
        assert!(
            !t.phase.program.node_facts().node_types.is_empty(),
            "expected node types to be baked onto the tree"
        );
    }
    ...
    fn binder_types_live_in_local_tys_not_node_types() {
        ...
        struct Binders {
            found: Vec<(
                talk_front::node_id::NodeID,
                talk_front::name_resolution::symbol::Symbol,
            )>,
        }
        impl Visitor for Binders {
            fn visit(&mut self, item: &dyn std::any::Any, event: derive_visitor::Event) {
                if matches!(event, derive_visitor::Event::Enter)
                    && let Some(pattern) = item.downcast_ref::<talk_front::typed_ast::Pattern>()
                {
        ...
        let facts = t.phase.program.node_facts();
        ...
        for (id, symbol) in binders.found {
            assert!(
                !facts.node_types.contains_key(&id),
                "binder node {id:?} unexpectedly has a baked node type"
            );
        ...
        assert!(
            t.phase
                ...
                .all(|resolution| !matches!(
                    resolution,
                    talk_front::types::output::MemberResolution::ViaRequirement { .. }
                ))
        );
    }
    ...
    fn auto_borrow_does_not_overwrite_argument_node_type() {
        ...
        assert_clean(&t);
        let facts = t.phase.program.node_facts();
        let borrowed_exprs: Vec<_> = facts
            .node_types
            .values()
    ...
    fn effect_rows_propagate_through_callers_and_payloads_zonk() {
        ...
        assert!(
            matches!(
                &sig.params[0],
                talk_front::types::ty::Ty::Nominal(sym, _)
                    if *sym == talk_front::name_resolution::symbol::Symbol::Int
            ),
    ...
    fn conformance_rows_commit_ordered_dictionaries() {
        ...
        use talk_front::types::catalog::DictionaryEntry;
        let t = check(
            "// no-core\nprotocol Greet {\n\tfunc hello() -> Int\n\tfunc again() -> Int {\n\t\tself.hello()\n\t}\n}\nextend Int: Greet {\n\tfunc hello() -> Int { 1 }\n}",
        );
        assert_clean(&t);
    ...
    fn derived_conformances_are_synthesized_rows_with_recipe_dictionaries() {
        ...
        use talk_front::name_resolution::symbol::Symbol;
        use talk_front::types::catalog::{DerivedRecipe, DictionaryEntry};
        let t = Driver::new(
    ...
    fn catalog_commits_callable_owner_bindings() {
        ...
        use talk_front::types::catalog::OwnerBinding;
        let t = check(
    ...
    fn typed_funcs_carry_receiver_and_binding_facts() {
        ...
        use talk_front::typed_ast::{DeclKind, ExprKind, Node, PatternKind};
        let t = check(
            "// no-core\nstruct S {\n\tlet v: Int\n\tmut func bump() -> Void { () }\n}\nfunc top() -> Int { 1 }",
        );
        assert_clean(&t);
    ...
    fn exported_catalog_carries_no_unification_variables() {
        ...
        fn ty_has_vars(ty: &talk_front::types::ty::Ty) -> bool {
            use talk_front::types::ty::{EffTail, RowTail, Ty};
            match ty {
                Ty::Var(_) => true,
                ...
                Ty::Static(talk_front::types::ty::StaticValue::Int(int)) => int
                    .terms
                    .iter()
                    .any(|(atom, _)| matches!(atom, talk_front::types::ty::StaticAtom::Var(_))),
                Ty::Static(_) => false,
    ...
    fn instantiations_recorded_at_call_sites() {
        ...
        assert_clean(&t);
        let facts = t.phase.program.node_facts();
        let instantiations = &facts.instantiations;
    ...
    fn gadt_variant_constructor_schemes_are_lowered() {
        ...
        let _names =
            talk_front::name_resolution::symbol::set_symbol_names(resolved.symbol_names.clone());
        let expr = resolved
            .symbol_names
            .iter()
            .find(|(sym, name)| {
...
mod with_core {
    ...
    fn struct_eff_params_cross_the_module_boundary() {
        ...
        let config = talk::compiling::driver::DriverConfig {
            module_id: ModuleId::Main,
            modules: Rc::new(modules),
            mode: talk::compiling::driver::CompilationMode::Library,
            module_name: "B".to_string(),
            parse_mode: talk::compiling::driver::ParseMode::Strict,
    ...
    fn rigid_slot_without_donation_evidence_reports_the_adaptation() {
        ...
        // (ADR 0054 stage 5).
        let t = check_with_core(Source::from(
            "func eat<T>(consume value: T) -> T { value }\nfunc caller<T>(value: &T) -> T {\n\teat(value: value)\n}",
        ));
        let errors = type_errors(&t);
        assert!(
            errors
    ...
    fn grades_derive_from_declarations() {
        use talk_front::name_resolution::symbol::Symbol;
        use talk_front::types::catalog::Grade;
        let t = check_with_core(Source::from(
            "struct FileHandle 'linear {\n\tlet fd: Int\n}\nstruct Plain {\n\tlet x: Int\n}\nextend Plain: Copy {}\nstruct Holder {\n\tlet name: String\n}",
        ));
        assert_no_errors(&t);
...
func f(g: Grid<4611686018427387904 * 4>) { }",
    ...
    fn static_int_normalization_canonicalizes_affine_forms() {
        use talk_front::front::module::ModuleId;
        use talk_front::name_resolution::symbol::{Symbol, TypeParameterId};
        use talk_front::types::ty::{StaticAtom, StaticInt, StaticValue, Ty};
...
mod nested_types {
    ...
    #[test]
    fn inferred_parameters_are_positional_unless_the_colon_is_written() {
        let positional = check("func id(x) { x }\nid(1)");
        assert_clean(&positional);

        let unexpected = check("func id(x) { x }\nid(x: 1)");
        let errors = type_errors(&unexpected);
        assert!(
            errors
                .iter()
        ...
        let labeled = check("func id(x:) { x }\nid(x: 1)");
        assert_clean(&labeled);

        let missing = check("func id(x:) { x }\nid(1)");
        let errors = type_errors(&missing);
        assert!(
            errors
                .iter()
                .any(|e| e.contains("Missing argument label 'x'")),
    ...
    fn all_diagnostics(
        driver: &talk::compiling::driver::Driver<talk::compiling::driver::Typed>,
    ) -> Vec<String> {
        driver
            .phase
            .diagnostics
    ...
    fn registers_callable_contracts() {
        ...
        use talk_front::types::callables::CallableRole;
