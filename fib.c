...
static unsigned char *talk_arena_chunk = NULL;
static size_t talk_arena_used = 0;
static size_t talk_arena_cap = 0;
static unsigned char **talk_arena_chunks = NULL;
static size_t talk_arena_count = 0;
static size_t talk_arena_capacity = 0;
...
static size_t talk_live_allocations;
...
static TalkValue talk_is_unique(TalkValue value) {
    ...
    return talk_bool(talk_header(value.v.ptr)->rc == 1);
}
...
static uintptr_t talk_stack_anchor;
static uintptr_t talk_stack_budget;
...
static uint32_t *talk_frames = NULL;
static size_t talk_depth = 0;
static size_t talk_frame_capacity = 0;
static uint32_t talk_next_frame_id = 1;

static int talk_unwinding = 0;
static size_t talk_unwind_depth = 0;
static uint32_t talk_unwind_frame = 0;
static TalkValue talk_unwind_value;
...
static TalkHandler *talk_handlers = NULL;
static size_t talk_handler_count = 0;
static size_t talk_handler_capacity = 0;
/* `SIZE_MAX` is "no floor", matching the VM's `handler_floor` start. */
static size_t talk_handler_floor = SIZE_MAX;
...
static uint64_t talk_next_ordinal;
static size_t talk_live_objects;
/* Regions whose count reached zero and whose walk has not finished. */
static TalkRegion **talk_pending;
static size_t talk_pending_count;
static size_t talk_pending_capacity;
...
static int talk_print(TalkValue value) {
    ...
    int owns_nothing = value.tag == TALK_UNIT || value.tag == TALK_BOOL
                       ...
                       || value.tag == TALK_FLOAT;
    if (owns_nothing && (talk_live_allocations != 0 || talk_live_objects != 0)) {
        fprintf(stderr, "talk: resource leak: %zu live allocations, %zu live objects at exit\n",
                talk_live_allocations, talk_live_objects);
        return 1;
    }
    return 0;
}
...
/* Immortal literal data; retaining or freeing a pointer into
   this blob is a no-op, as provenance zero is in the VM. */
static unsigned char talk_statics[] = {
    116, 97, 108, 107, 116, 97, 108, 107, 104, 101, 108, 108, 111, 32, 48, 45,
    10, 49, 50, 51, 52, 53, 54, 55, 56, 57,
};
static const char *const talk_members_1[] = { "lower", "upper", "closed", "current", "done" };
static const char *const talk_members_2[] = { "storage", "byte_count", "capacity" };
static const char *const talk_members_3[] = { "base" };
static const char *const talk_members_4[] = { "lower", "upper" };
static const char *const talk_members_5[] = { "some", "none" };
static const char *const talk_members_6[] = { "read", "write", "open", "close", "sleep", "poll", "ctl", "socket", "bind", "listen", "connect", "accept", "cwd_len", "cwd_copy", "getenv_len", "getenv_copy", "argc", "arg_len", "arg_copy", "dir_count", "dir_entry_kind", "dir_entry_len", "dir_entry_copy", "exit" };
static const TalkTypeInfo talk_type_table[] = {
    { "", TALK_TYPE_TUPLE, 0, NULL },
    { "RangeIterator", TALK_TYPE_RECORD, 5, talk_members_1 },
    { "String", TALK_TYPE_STRING, 3, talk_members_2 },
    { "ByteStorage", TALK_TYPE_RECORD, 1, talk_members_3 },
    { "Range", TALK_TYPE_RECORD, 2, talk_members_4 },
    { "Optional", TALK_TYPE_ENUM, 2, talk_members_5 },
    { "IORequest", TALK_TYPE_ENUM, 24, talk_members_6 },
};
static const TalkField talk_lfields_0[] = { { 0, 1, UINT32_MAX, 0 } };
static const TalkField talk_lfields_1[] = { { 0, 1, 0u, 3 }, { 1, 1, UINT32_MAX, 0 }, { 2, 1, UINT32_MAX, 0 } };
static const TalkField talk_lfields_2[] = { { 0, 1, UINT32_MAX, 0 }, { 1, 1, UINT32_MAX, 0 } };
static const TalkField talk_lfields_3[] = { { 1, 1, UINT32_MAX, 0 } };
static const uint32_t talk_lvars_3[] = { 0, 1, 1 };
static const TalkField talk_lfields_4[] = { { 0, 2, 3u, 5 }, { 2, 1, UINT32_MAX, 0 } };
static const TalkField talk_lfields_6[] = { { 1, 1, UINT32_MAX, 0 }, { 2, 1, UINT32_MAX, 0 }, { 3, 1, UINT32_MAX, 0 }, { 1, 1, UINT32_MAX, 0 }, { 2, 1, UINT32_MAX, 0 }, { 3, 1, UINT32_MAX, 0 }, { 1, 1, UINT32_MAX, 0 }, { 2, 1, UINT32_MAX, 0 }, { 3, 1, UINT32_MAX, 0 }, { 1, 1, UINT32_MAX, 0 }, { 1, 1, UINT32_MAX, 0 }, { 1, 1, UINT32_MAX, 0 }, { 2, 1, UINT32_MAX, 0 }, { 3, 1, UINT32_MAX, 0 }, { 1, 1, UINT32_MAX, 0 }, { 2, 1, UINT32_MAX, 0 }, { 3, 1, UINT32_MAX, 0 }, { 1, 1, UINT32_MAX, 0 }, { 2, 1, UINT32_MAX, 0 }, { 3, 1, UINT32_MAX, 0 }, { 1, 1, UINT32_MAX, 0 }, { 2, 1, UINT32_MAX, 0 }, { 3, 1, UINT32_MAX, 0 }, { 1, 1, UINT32_MAX, 0 }, { 2, 1, UINT32_MAX, 0 }, { 1, 1, UINT32_MAX, 0 }, { 2, 1, UINT32_MAX, 0 }, { 3, 1, UINT32_MAX, 0 }, { 1, 1, UINT32_MAX, 0 }, { 1, 1, UINT32_MAX, 0 }, { 1, 1, UINT32_MAX, 0 }, { 2, 1, UINT32_MAX, 0 }, { 1, 1, UINT32_MAX, 0 }, { 2, 1, UINT32_MAX, 0 }, { 3, 1, UINT32_MAX, 0 }, { 1, 1, UINT32_MAX, 0 }, { 1, 1, UINT32_MAX, 0 }, { 2, 1, UINT32_MAX, 0 }, { 1, 1, UINT32_MAX, 0 }, { 1, 1, UINT32_MAX, 0 }, { 2, 1, UINT32_MAX, 0 }, { 1, 1, UINT32_MAX, 0 }, { 2, 1, UINT32_MAX, 0 }, { 1, 1, UINT32_MAX, 0 }, { 2, 1, UINT32_MAX, 0 }, { 3, 1, UINT32_MAX, 0 }, { 1, 1, UINT32_MAX, 0 } };
static const uint32_t talk_lvars_6[] = { 0, 3, 6, 9, 10, 11, 14, 17, 20, 23, 25, 28, 29, 29, 30, 32, 35, 35, 36, 38, 39, 41, 43, 46, 47 };
static const TalkField talk_lfields_7[] = { { 0, 1, UINT32_MAX, 0 }, { 1, 1, UINT32_MAX, 0 }, { 2, 1, UINT32_MAX, 0 }, { 3, 1, UINT32_MAX, 0 }, { 4, 1, UINT32_MAX, 0 } };
static const TalkLayoutInfo talk_layout_table[] = {
    { 1, 0, talk_lfields_0, 1, NULL, 0 },
    { 3, 0, talk_lfields_1, 3, NULL, 0 },
    { 2, 0, talk_lfields_2, 2, NULL, 0 },
    { 2, 1, talk_lfields_3, 1, talk_lvars_3, 2 },
    { 3, 0, talk_lfields_4, 2, NULL, 0 },
    { 1, 0, NULL, 0, NULL, 0 },
    { 4, 1, talk_lfields_6, 47, talk_lvars_6, 24 },
    { 5, 0, talk_lfields_7, 5, NULL, 0 },
};

typedef struct {
    unsigned char * m0;
} TalkL0;
static inline TalkValue talk_box_l0(TalkL0 v) {
    TalkValue built = talk_native_box(0u, 3, sizeof(TalkL0));
    *(TalkL0 *)TALK_NATIVE_PAYLOAD(built) = v;
    return built;
}
static inline TalkValue talk_box_l0_in(void *storage, TalkL0 v) {
    TalkValue built = talk_native_box_in(storage, 0u, 3);
    *(TalkL0 *)TALK_NATIVE_PAYLOAD(built) = v;
    return built;
}
static inline TalkL0 talk_unbox_l0(TalkValue b) {
    if (b.tag == TALK_NATIVE) {
        return *(const TalkL0 *)TALK_NATIVE_PAYLOAD(b);
    }
    TalkL0 v;
    v.m0 = b.v.agg->fields[0].v.ptr;
    return v;
}
static TalkValue talk_retag_l0(TalkL0 v) {
    (void)v;
    TalkValue built = talk_agg(0u, 3, 0, 1);
    built.v.agg->fields[0] = talk_pointer(v.m0);
    return built;
}
...
static inline TalkValue talk_box_l1(TalkL1 v) {
    TalkValue built = talk_native_box(1u, 2, sizeof(TalkL1));
    *(TalkL1 *)TALK_NATIVE_PAYLOAD(built) = v;
    return built;
}
static inline TalkValue talk_box_l1_in(void *storage, TalkL1 v) {
    TalkValue built = talk_native_box_in(storage, 1u, 2);
    *(TalkL1 *)TALK_NATIVE_PAYLOAD(built) = v;
    return built;
}
static inline TalkL1 talk_unbox_l1(TalkValue b) {
    if (b.tag == TALK_NATIVE) {
        return *(const TalkL1 *)TALK_NATIVE_PAYLOAD(b);
    }
    TalkL1 v;
    v.m0 = b.v.agg->fields[0].v.ptr;
    v.m1 = b.v.agg->fields[1].v.i;
    v.m2 = b.v.agg->fields[2].v.i;
    return v;
}
static TalkValue talk_retag_l1(TalkL1 v) {
    (void)v;
    TalkValue built = talk_agg(1u, 2, 0, 3);
    built.v.agg->fields[0] = talk_pointer(v.m0);
    built.v.agg->fields[1] = talk_int(v.m1);
    built.v.agg->fields[2] = talk_int(v.m2);
    return built;
}
...
static inline TalkValue talk_box_l2(TalkL2 v) {
    TalkValue built = talk_native_box(2u, 4, sizeof(TalkL2));
    *(TalkL2 *)TALK_NATIVE_PAYLOAD(built) = v;
    return built;
}
static inline TalkValue talk_box_l2_in(void *storage, TalkL2 v) {
    TalkValue built = talk_native_box_in(storage, 2u, 4);
    *(TalkL2 *)TALK_NATIVE_PAYLOAD(built) = v;
    return built;
}
static inline TalkL2 talk_unbox_l2(TalkValue b) {
    if (b.tag == TALK_NATIVE) {
        return *(const TalkL2 *)TALK_NATIVE_PAYLOAD(b);
    }
    TalkL2 v;
...
static TalkValue talk_retag_l2(TalkL2 v) {
    (void)v;
    TalkValue built = talk_agg(2u, 4, 0, 2);
...
static inline TalkValue talk_box_l3(TalkL3 v) {
    TalkValue built = talk_agg(3u, 5, 0, 2);
    built.v.agg->fields[0] = talk_int(v.m0);
    built.v.agg->fields[1] = talk_int(v.m1);
    return built;
}
static inline TalkValue talk_box_l3_in(void *storage, TalkL3 v) {
    TalkValue built = talk_agg_in(storage, 3u, 5, 0, 2);
    built.v.agg->fields[0] = talk_int(v.m0);
    built.v.agg->fields[1] = talk_int(v.m1);
    return built;
}
static inline TalkL3 talk_unbox_l3(TalkValue b) {
    TalkL3 v;
    v.m0 = b.v.agg->fields[0].v.i;
    v.m1 = b.v.agg->fields[1].v.i;
    return v;
...
static inline TalkValue talk_box_l4(TalkL4 v) {
    TalkValue built = talk_native_box(4u, 0, sizeof(TalkL4));
    *(TalkL4 *)TALK_NATIVE_PAYLOAD(built) = v;
    return built;
}
static inline TalkValue talk_box_l4_in(void *storage, TalkL4 v) {
    TalkValue built = talk_native_box_in(storage, 4u, 0);
    *(TalkL4 *)TALK_NATIVE_PAYLOAD(built) = v;
    return built;
}
static inline TalkL4 talk_unbox_l4(TalkValue b) {
    if (b.tag == TALK_NATIVE) {
        return *(const TalkL4 *)TALK_NATIVE_PAYLOAD(b);
    }
    TalkL4 v;
    v.m0 = b.v.agg->fields[0].v.i;
    v.m1 = b.v.agg->fields[1].v.i;
    v.m2 = b.v.agg->fields[2];
    return v;
}
static TalkValue talk_retag_l4(TalkL4 v) {
    (void)v;
    TalkValue built = talk_agg(4u, 0, 0, 3);
    built.v.agg->fields[0] = talk_int(v.m0);
    built.v.agg->fields[1] = talk_int(v.m1);
    built.v.agg->fields[2] = v.m2;
    return built;
}
static void talk_scan_l4(const TalkL4 *v, struct TalkObject ***scan_out,
                            size_t *scan_count, size_t *scan_capacity) {
    talk_scan_handles(v->m2, scan_out, scan_count, scan_capacity);
}
...
static inline TalkValue talk_box_l6(TalkL6 v) {
    TalkValue built = talk_agg(6u, 6, 0, 4);
    built.v.agg->fields[0] = talk_int(v.m0);
    built.v.agg->fields[1] = v.m1;
    built.v.agg->fields[2] = v.m2;
    built.v.agg->fields[3] = v.m3;
    return built;
}
static inline TalkValue talk_box_l6_in(void *storage, TalkL6 v) {
    TalkValue built = talk_agg_in(storage, 6u, 6, 0, 4);
    built.v.agg->fields[0] = talk_int(v.m0);
    built.v.agg->fields[1] = v.m1;
    built.v.agg->fields[2] = v.m2;
    built.v.agg->fields[3] = v.m3;
...
static inline TalkL6 talk_unbox_l6(TalkValue b) {
    TalkL6 v;
    v.m0 = b.v.agg->fields[0].v.i;
    v.m1 = b.v.agg->fields[1];
    v.m2 = b.v.agg->fields[2];
    v.m3 = b.v.agg->fields[3];
    return v;
}
...
static inline TalkValue talk_box_l7(TalkL7 v) {
    TalkValue built = talk_native_box(7u, 1, sizeof(TalkL7));
    *(TalkL7 *)TALK_NATIVE_PAYLOAD(built) = v;
    return built;
}
static inline TalkValue talk_box_l7_in(void *storage, TalkL7 v) {
    TalkValue built = talk_native_box_in(storage, 7u, 1);
    *(TalkL7 *)TALK_NATIVE_PAYLOAD(built) = v;
    return built;
}
static inline TalkL7 talk_unbox_l7(TalkValue b) {
    if (b.tag == TALK_NATIVE) {
        return *(const TalkL7 *)TALK_NATIVE_PAYLOAD(b);
    }
    TalkL7 v;
    v.m0 = b.v.agg->fields[0].v.i;
    v.m1 = b.v.agg->fields[1].v.i;
    v.m2 = b.v.agg->fields[2].v.i;
    v.m3 = b.v.agg->fields[3].v.i;
    v.m4 = b.v.agg->fields[4].v.i;
    return v;
}
static TalkValue talk_retag_l7(TalkL7 v) {
    (void)v;
    TalkValue built = talk_agg(7u, 1, 0, 5);
    built.v.agg->fields[0] = talk_int(v.m0);
    built.v.agg->fields[1] = talk_int(v.m1);
    built.v.agg->fields[2] = talk_bool(v.m2);
    built.v.agg->fields[3] = talk_int(v.m3);
    built.v.agg->fields[4] = talk_bool(v.m4);
    return built;
}
...
static TalkValue talk_native_retag(TalkValue value) {
    ...
    case 1: return talk_retag_l1(*(const TalkL1 *)TALK_NATIVE_PAYLOAD(value));
    case 2: return talk_retag_l2(*(const TalkL2 *)TALK_NATIVE_PAYLOAD(value));
    case 4: return talk_retag_l4(*(const TalkL4 *)TALK_NATIVE_PAYLOAD(value));
    case 7: return talk_retag_l7(*(const TalkL7 *)TALK_NATIVE_PAYLOAD(value));
    }
    return talk_unit();
}
...
static TalkValue talk_rebox(uint32_t layout, TalkValue flat) {
    switch (layout) {
    case 0: return talk_box_l0(talk_unbox_l0(flat));
    case 1: return talk_box_l1(talk_unbox_l1(flat));
    case 2: return talk_box_l2(talk_unbox_l2(flat));
    case 4: return talk_box_l4(talk_unbox_l4(flat));
    case 7: return talk_box_l7(talk_unbox_l7(flat));
    }
    return flat;
}
...
static void talk_native_scan(TalkValue value, struct TalkObject ***out, size_t *count,
                             size_t *capacity) {
    switch (value.v.native->layout) {
    case 4: talk_scan_l4((const TalkL4 *)TALK_NATIVE_PAYLOAD(value), out, count, capacity); break;
    default: break;
    }
}

/* Program globals: one 8-byte boxed slot each, zero meaning
   uninitialized so a read before the initializer traps. */
static unsigned char talk_globals[8];

static TalkValue talk_fn0(const TalkValue *env); /* script */
static TalkValue talk_fn1(const TalkValue *env, TalkL1 p0); /* greet */
static TalkValue talk_fn2(const TalkValue *env, TalkL2 p0); /* iter */
static TalkL4 talk_fn3(const TalkValue *env, TalkValue p0); /* next */
static TalkValue talk_fn4(const TalkValue *env, TalkValue p0); /* fib */
static TalkValue talk_fn5(const TalkValue *env, TalkValue p0); /* print */
static TalkValue talk_fn6(const TalkValue *env, TalkValue p0); /* _with_host */
static TalkValue talk_fn7(const TalkValue *env); /* entry_body */
static TalkValue talk_fn8(const TalkValue *env); /* script_main */
static TalkL1 talk_fn9(const TalkValue *env, TalkValue p0); /* show */
static TalkValue talk_fn10(const TalkValue *env, TalkL1 p0); /* _print_line */
static TalkL1 talk_fn11(const TalkValue *env, TalkValue p0); /* _digit */
static TalkL1 talk_fn12(const TalkValue *env, TalkL1 p0, TalkL1 p1); /* add */
static TalkValue talk_fn13(const TalkValue *env, TalkValue p0); /* _alloc */
static TalkL1 talk_fn14(const TalkValue *env, TalkValue p0, TalkValue p1, TalkValue p2, TalkValue p3); /* init */
static TalkValue talk_fn15(const TalkValue *env, TalkValue p0, TalkValue p1, TalkValue p2, TalkValue p3); /* init */
static TalkValue talk_fn16(const TalkValue *env, TalkL1 p0); /* print */
static TalkL1 talk_fn17(const TalkValue *env, TalkL1 p0); /* to_string */
...
static TalkValue talk_fn0(const TalkValue *env) {
    ...
    _Alignas(TalkValue) unsigned char fx2[sizeof(TalkAgg) + 2 * sizeof(TalkValue)];
    (void)fx2;
    TalkL4 x3;
...
b0:
    {
        TalkL1 tmp;
        tmp.m0 = talk_statics + 0;
        tmp.m1 = 8;
        tmp.m2 = 8;
        l[0] = talk_box_l1(tmp);
    }
    l[1] = talk_fn1(NULL, talk_unbox_l1(l[0]));
    if (talk_unwinding) {
        return talk_unit();
    }
    x2.m0 = talk_int(INT64_C(0)).v.i;
    x2.m1 = talk_int(INT64_C(10)).v.i;
    l[1] = talk_fn2(NULL, x2);
    if (talk_unwinding) {
        return talk_unit();
    }
    goto b1;
...
b3:
    ...
    goto b4;
b4:
    l[5] = talk_fn4(NULL, l[0]);
    if (talk_unwinding) {
        return talk_unit();
    }
    l[6] = talk_fn5(NULL, l[5]);
    if (talk_unwinding) {
        return talk_unit();
    }
    l[0] = talk_add(l[0], talk_int(INT64_C(1)));
    goto b5;
b5:
    goto b1;
b6:
...
b8:
    goto b9;
b9:
    l[1] = talk_fn4(NULL, talk_int(INT64_C(10)));
    if (talk_unwinding) {
        return talk_unit();
    }
    l[0] = talk_fn5(NULL, l[1]);
    if (talk_unwinding) {
        return talk_unit();
    }
    return l[0];
...
/* greet */
static TalkValue talk_fn1(const TalkValue *env, TalkL1 p0) {
    TalkValue l[4];
    memset(l, 0, sizeof l);
    (void)env;
    TalkL1 x0;
...
b0:
    {
        TalkL1 tmp;
        tmp.m0 = talk_statics + 8;
        tmp.m1 = 6;
        tmp.m2 = 6;
        l[1] = talk_box_l1(tmp);
    }
    x2 = talk_fn12(NULL, talk_unbox_l1(l[1]), x0);
    if (talk_unwinding) {
        return talk_unit();
    }
    l[1] = talk_fn16(NULL, x2);
    if (talk_unwinding) {
        goto b1;
    }
    l[3] = talk_pointer(x2.m0);
    talk_free(l[3]);
    return l[1];
b1:
    l[1] = talk_pointer(x2.m0);
    talk_free(l[1]);
    return talk_unit();
}

/* iter */
static TalkValue talk_fn2(const TalkValue *env, TalkL2 p0) {
    TalkValue l[5];
    memset(l, 0, sizeof l);
    (void)env;
    TalkL2 x0;
...
b0:
    ...
    {
        TalkValue built = talk_native_box(7u, 1, sizeof(TalkL7));
        memset(TALK_NATIVE_PAYLOAD(built), 0, sizeof(TalkL7));
        l[3] = built;
    }
    l[4] = talk_fn15(NULL, l[3], l[1], l[2], talk_bool(0));
    if (talk_unwinding) {
        return talk_unit();
    }
    return l[4];
...
/* next */
static TalkL4 talk_fn3(const TalkValue *env, TalkValue p0) {
    TalkValue l[6];
    memset(l, 0, sizeof l);
    (void)env;
    TalkL3 x2;
    ...
    goto b0;
b0:
    l[1] = talk_bool(((const TalkL7 *)TALK_NATIVE_PAYLOAD(l[0]))->m4);
    if (l[1].v.i) goto b1; else goto b2;
b1:
    x2.m0 = 1;
    x3.m0 = x2.m0;
    x3.m1 = x2.m1;
...
b2:
    goto b3;
b3:
    l[1] = talk_int(((const TalkL7 *)TALK_NATIVE_PAYLOAD(l[0]))->m3);
    l[4] = talk_int(((const TalkL7 *)TALK_NATIVE_PAYLOAD(l[0]))->m1);
    l[5] = talk_cmp_lt(l[1], l[4]);
    if (l[5].v.i) goto b4; else goto b5;
b4:
    l[5] = talk_add(l[1], talk_int(INT64_C(1)));
    {
        TalkValue copy = talk_box_l7(talk_unbox_l7(l[0]));
        TalkL7 *v = (TalkL7 *)TALK_NATIVE_PAYLOAD(copy);
        v->m3 = l[5].v.i;
        l[0] = copy;
    }
    x2.m0 = 0;
...
b6:
    {
        TalkValue copy = talk_box_l7(talk_unbox_l7(l[0]));
        TalkL7 *v = (TalkL7 *)TALK_NATIVE_PAYLOAD(copy);
        v->m4 = talk_bool(1).v.i;
        l[0] = copy;
    }
    l[5] = talk_bool(((const TalkL7 *)TALK_NATIVE_PAYLOAD(l[0]))->m2);
    if (l[5].v.i) goto b7; else goto b8;
b7:
    l[5] = talk_int(((const TalkL7 *)TALK_NATIVE_PAYLOAD(l[0]))->m1);
    l[4] = talk_cmp_eq(l[1], l[5]);
    {
        TalkValue e0 = l[4];
        l[4] = e0;
        goto b9;
...
/* fib */
static TalkValue talk_fn4(const TalkValue *env, TalkValue p0) {
    TalkValue l[3];
    memset(l, 0, sizeof l);
    (void)env;
    talk_frame_enter();
    ...
    goto b0;
b0:
    l[1] = talk_cmp_le(l[0], talk_int(INT64_C(1)));
    if (l[1].v.i) goto b1; else goto b2;
b1:
    return l[0];
b2:
    goto b3;
b3:
    l[1] = talk_sub(l[0], talk_int(INT64_C(2)));
    l[2] = talk_fn4(NULL, l[1]);
    if (talk_unwinding) {
        return talk_unit();
    }
    l[1] = talk_sub(l[0], talk_int(INT64_C(1)));
    l[0] = talk_fn4(NULL, l[1]);
    if (talk_unwinding) {
        return talk_unit();
    }
    l[1] = talk_add(l[2], l[0]);
    return l[1];
}

/* print */
static TalkValue talk_fn5(const TalkValue *env, TalkValue p0) {
    TalkValue l[3];
    memset(l, 0, sizeof l);
    (void)env;
    TalkL1 x1;
    ...
    goto b0;
b0:
    x1 = talk_fn9(NULL, l[0]);
    if (talk_unwinding) {
        return talk_unit();
    }
    l[0] = talk_fn10(NULL, x1);
    if (talk_unwinding) {
        goto b1;
    }
    l[2] = talk_pointer(x1.m0);
    talk_free(l[2]);
    return l[0];
b1:
    l[0] = talk_pointer(x1.m0);
    talk_free(l[0]);
    return talk_unit();
}

/* _with_host */
static TalkValue talk_fn6(const TalkValue *env, TalkValue p0) {
    TalkValue l[2];
    memset(l, 0, sizeof l);
    (void)env;
    talk_frame_enter();
...
b0:
    {
        TalkValue a[4];
        memset(a + 0, 0, sizeof(TalkValue) * 4);
        l[1] = talk_dispatch(l[0].v.agg->meta, l[0].v.agg->fields, a);
    }
    if (talk_unwinding) {
        return talk_unit();
    }
...
/* entry_body */
static TalkValue talk_fn7(const TalkValue *env) {
    TalkValue l[1];
    memset(l, 0, sizeof l);
    (void)env;
    talk_frame_enter();
...
b0:
    ...
    talk_store_boxed(talk_pointer(talk_globals + 0), l[0]);
    return talk_unit();
}

/* script_main */
static TalkValue talk_fn8(const TalkValue *env) {
    TalkValue l[2];
    memset(l, 0, sizeof l);
    (void)env;
    talk_frame_enter();
...
b0:
    {
        TalkValue built = talk_closure(7, 0);
        l[0] = built;
    }
    l[1] = talk_fn6(NULL, l[0]);
    if (talk_unwinding) {
        return talk_unit();
    }
    l[1] = talk_load_boxed(talk_pointer(talk_globals + 0));
    return l[1];
}

/* show */
static TalkL1 talk_fn9(const TalkValue *env, TalkValue p0) {
    TalkValue l[8];
    memset(l, 0, sizeof l);
    (void)env;
    TalkL1 x6;
    ...
    goto b0;
b0:
    l[1] = talk_cmp_eq(l[0], talk_int(INT64_C(0)));
    if (l[1].v.i) goto b1; else goto b2;
b1:
    {
        TalkL1 tmp;
        tmp.m0 = talk_statics + 14;
        tmp.m1 = 1;
        tmp.m2 = 1;
        l[1] = talk_box_l1(tmp);
    }
...
b3:
    l[1] = l[0];
    l[2] = talk_cmp_lt(l[0], talk_int(INT64_C(0)));
    if (l[2].v.i) goto b4; else goto b5;
b4:
    l[1] = talk_sub(talk_int(INT64_C(0)), l[0]);
    goto b6;
b5:
    goto b6;
b6:
    {
        TalkL1 tmp;
        tmp.m0 = talk_statics + 15;
        tmp.m1 = 0;
        tmp.m2 = 0;
        l[2] = talk_box_l1(tmp);
    }
    l[3] = l[2];
    l[4] = talk_pointer(((const TalkL1 *)TALK_NATIVE_PAYLOAD(l[2]))->m0);
    talk_retain(l[4]);
    goto b7;
b7:
    l[4] = talk_cmp_gt(l[1], talk_int(INT64_C(0)));
    if (l[4].v.i) goto b8; else goto b11;
b8:
    l[4] = talk_div(l[1], talk_int(INT64_C(10)));
    l[5] = talk_mul(l[4], talk_int(INT64_C(10)));
    l[4] = talk_sub(l[1], l[5]);
    x6 = talk_fn11(NULL, l[4]);
    if (talk_unwinding) {
        goto b14;
    }
    x7 = talk_fn12(NULL, x6, talk_unbox_l1(l[3]));
    if (talk_unwinding) {
        goto b9;
    }
    l[4] = talk_pointer(((const TalkL1 *)TALK_NATIVE_PAYLOAD(l[3]))->m0);
    talk_free(l[4]);
    l[3] = talk_box_l1(x7);
    l[1] = talk_div(l[1], talk_int(INT64_C(10)));
    goto b10;
b9:
    l[4] = talk_pointer(x6.m0);
    talk_free(l[4]);
    goto b14;
b10:
    l[4] = talk_pointer(x6.m0);
...
b11:
    goto b12;
b12:
    l[1] = talk_cmp_lt(l[0], talk_int(INT64_C(0)));
    if (l[1].v.i) goto b13; else goto b15;
b13:
    {
        TalkL1 tmp;
        tmp.m0 = talk_statics + 15;
    ...
    x6 = talk_fn12(NULL, talk_unbox_l1(l[1]), talk_unbox_l1(l[3]));
    if (talk_unwinding) {
        goto b14;
    }
    {
        TalkValue e0 = talk_box_l1(x6);
        l[2] = e0;
        goto b16;
    }
b14:
    l[1] = talk_pointer(((const TalkL1 *)TALK_NATIVE_PAYLOAD(l[3]))->m0);
    talk_free(l[1]);
    return (TalkL1){0};
b15:
    l[1] = talk_pointer(((const TalkL1 *)TALK_NATIVE_PAYLOAD(l[3]))->m0);
...
b16:
    l[4] = talk_pointer(((const TalkL1 *)TALK_NATIVE_PAYLOAD(l[3]))->m0);
    talk_free(l[4]);
    return talk_unbox_l1(l[2]);
}

/* _print_line */
static TalkValue talk_fn10(const TalkValue *env, TalkL1 p0) {
    TalkValue l[6];
    memset(l, 0, sizeof l);
    (void)env;
    TalkL1 x0;
    ...
    goto b0;
b0:
    l[1] = talk_pointer(x0.m0);
    l[2] = talk_int(x0.m1);
    l[3] = talk_int(talk_io(1, talk_int(INT64_C(1)), l[1], l[2]));
    {
        TalkL1 tmp;
        tmp.m0 = talk_statics + 16;
        tmp.m1 = 1;
        tmp.m2 = 1;
        l[3] = talk_box_l1(tmp);
    }
    ...
    l[1] = talk_pointer(((const TalkL1 *)TALK_NATIVE_PAYLOAD(l[3]))->m0);
    talk_retain(l[1]);
    l[1] = talk_pointer(((const TalkL1 *)TALK_NATIVE_PAYLOAD(l[2]))->m0);
    l[4] = talk_int(((const TalkL1 *)TALK_NATIVE_PAYLOAD(l[2]))->m1);
    l[5] = talk_int(talk_io(1, talk_int(INT64_C(1)), l[1], l[4]));
    l[5] = talk_pointer(((const TalkL1 *)TALK_NATIVE_PAYLOAD(l[2]))->m0);
    talk_free(l[5]);
    return talk_unit();
}

/* _digit */
static TalkL1 talk_fn11(const TalkValue *env, TalkValue p0) {
    TalkValue l[3];
    memset(l, 0, sizeof l);
    (void)env;
    talk_frame_enter();
...
b2:
    l[2] = talk_cmp_eq(l[0], talk_int(INT64_C(1)));
    if (l[2].v.i) goto b3; else goto b4;
b3:
    {
        TalkL1 tmp;
        tmp.m0 = talk_statics + 17;
        tmp.m1 = 1;
        tmp.m2 = 1;
        l[1] = talk_box_l1(tmp);
    }
...
b4:
    l[2] = talk_cmp_eq(l[0], talk_int(INT64_C(2)));
    if (l[2].v.i) goto b5; else goto b6;
b5:
    {
        TalkL1 tmp;
        tmp.m0 = talk_statics + 18;
        tmp.m1 = 1;
        tmp.m2 = 1;
        l[1] = talk_box_l1(tmp);
    }
...
b6:
    l[2] = talk_cmp_eq(l[0], talk_int(INT64_C(3)));
    if (l[2].v.i) goto b7; else goto b8;
b7:
    {
        TalkL1 tmp;
        tmp.m0 = talk_statics + 19;
        tmp.m1 = 1;
        tmp.m2 = 1;
        l[1] = talk_box_l1(tmp);
    }
...
b8:
    l[2] = talk_cmp_eq(l[0], talk_int(INT64_C(4)));
    if (l[2].v.i) goto b9; else goto b10;
b9:
    {
        TalkL1 tmp;
        tmp.m0 = talk_statics + 20;
        tmp.m1 = 1;
        tmp.m2 = 1;
        l[1] = talk_box_l1(tmp);
    }
...
b10:
    l[2] = talk_cmp_eq(l[0], talk_int(INT64_C(5)));
    if (l[2].v.i) goto b11; else goto b12;
b11:
    {
        TalkL1 tmp;
        tmp.m0 = talk_statics + 21;
        tmp.m1 = 1;
        tmp.m2 = 1;
        l[1] = talk_box_l1(tmp);
    }
...
b12:
    l[2] = talk_cmp_eq(l[0], talk_int(INT64_C(6)));
    if (l[2].v.i) goto b13; else goto b14;
b13:
    {
        TalkL1 tmp;
        tmp.m0 = talk_statics + 22;
        tmp.m1 = 1;
        tmp.m2 = 1;
        l[1] = talk_box_l1(tmp);
    }
...
b14:
    l[2] = talk_cmp_eq(l[0], talk_int(INT64_C(7)));
    if (l[2].v.i) goto b15; else goto b16;
b15:
    {
        TalkL1 tmp;
        tmp.m0 = talk_statics + 23;
        tmp.m1 = 1;
        tmp.m2 = 1;
        l[1] = talk_box_l1(tmp);
    }
...
b16:
    l[2] = talk_cmp_eq(l[0], talk_int(INT64_C(8)));
    if (l[2].v.i) goto b17; else goto b18;
b17:
    {
        TalkL1 tmp;
        tmp.m0 = talk_statics + 24;
        tmp.m1 = 1;
        tmp.m2 = 1;
        l[1] = talk_box_l1(tmp);
    }
...
/* add */
static TalkL1 talk_fn12(const TalkValue *env, TalkL1 p0, TalkL1 p1) {
    TalkValue l[7];
    memset(l, 0, sizeof l);
    (void)env;
    TalkL1 x0;
...
b0:
    l[2] = talk_int(x0.m1);
    l[3] = talk_int(x1.m1);
    l[4] = talk_add(l[2], l[3]);
    l[5] = talk_fn13(NULL, l[4]);
    if (talk_unwinding) {
        return (TalkL1){0};
    }
    l[6] = talk_pointer(x0.m0);
    talk_mem_copy(l[6], l[5], l[2]);
    l[6] = talk_ptr_add(l[5], l[2], 1);
    l[2] = talk_pointer(x1.m0);
    talk_mem_copy(l[2], l[6], l[3]);
    {
        TalkValue built = talk_native_box(1u, 2, sizeof(TalkL1));
        memset(TALK_NATIVE_PAYLOAD(built), 0, sizeof(TalkL1));
        l[6] = built;
    }
    x1 = talk_fn14(NULL, l[6], l[5], l[4], l[4]);
    if (talk_unwinding) {
        return (TalkL1){0};
    }
    return x1;
...
/* _alloc */
static TalkValue talk_fn13(const TalkValue *env, TalkValue p0) {
    TalkValue l[2];
    memset(l, 0, sizeof l);
    (void)env;
    talk_frame_enter();
...
/* init */
static TalkL1 talk_fn14(const TalkValue *env, TalkValue p0, TalkValue p1, TalkValue p2, TalkValue p3) {
    TalkValue l[5];
    memset(l, 0, sizeof l);
    (void)env;
    TalkL0 x4;
...
b0:
    x4.m0 = l[1].v.ptr;
    {
        TalkValue copy = talk_box_l1(talk_unbox_l1(l[0]));
        TalkL1 *v = (TalkL1 *)TALK_NATIVE_PAYLOAD(copy);
        v->m0 = x4.m0;
        l[0] = copy;
    }
    {
    ...
    return talk_unbox_l1(l[0]);
}

/* init */
static TalkValue talk_fn15(const TalkValue *env, TalkValue p0, TalkValue p1, TalkValue p2, TalkValue p3) {
    TalkValue l[4];
    memset(l, 0, sizeof l);
    (void)env;
    talk_frame_enter();
...
/* print */
static TalkValue talk_fn16(const TalkValue *env, TalkL1 p0) {
    TalkValue l[4];
    memset(l, 0, sizeof l);
    (void)env;
    TalkL1 x0;
    ...
    goto b0;
b0:
    x1 = talk_fn17(NULL, x0);
    if (talk_unwinding) {
        return talk_unit();
    }
    l[2] = talk_fn10(NULL, x1);
    if (talk_unwinding) {
        goto b1;
    }
    l[3] = talk_pointer(x1.m0);
    talk_free(l[3]);
    return l[2];
b1:
    l[2] = talk_pointer(x1.m0);
...
/* to_string */
static TalkL1 talk_fn17(const TalkValue *env, TalkL1 p0) {
    TalkValue l[5];
    memset(l, 0, sizeof l);
    (void)env;
    TalkL1 x0;
...
b0:
    l[1] = talk_int(x0.m1);
    l[2] = talk_fn13(NULL, l[1]);
    if (talk_unwinding) {
        return (TalkL1){0};
    }
    l[1] = talk_pointer(x0.m0);
    l[3] = talk_int(x0.m1);
    talk_mem_copy(l[1], l[2], l[3]);
    l[3] = talk_int(x0.m1);
    l[1] = talk_int(x0.m1);
    {
        TalkValue built = talk_native_box(1u, 2, sizeof(TalkL1));
    ...
    x0 = talk_fn14(NULL, l[4], l[2], l[3], l[1]);
    if (talk_unwinding) {
        return (TalkL1){0};
    }
    return x0;
...
static TalkValue talk_dispatch(uint32_t function, const TalkValue *env, const TalkValue *args) {
    (void)args;
    switch (function) {
    case 0: return talk_fn0(env);
    case 1: return talk_fn1(env, talk_unbox_l1(args[0]));
    case 2: return talk_fn2(env, talk_unbox_l2(args[0]));
    case 3: return talk_box_l4(talk_fn3(env, args[0]));
    case 4: return talk_fn4(env, args[0]);
    case 5: return talk_fn5(env, args[0]);
    case 6: return talk_fn6(env, args[0]);
    case 7: return talk_fn7(env);
    case 8: return talk_fn8(env);
    case 9: return talk_box_l1(talk_fn9(env, args[0]));
    case 10: return talk_fn10(env, talk_unbox_l1(args[0]));
    case 11: return talk_box_l1(talk_fn11(env, args[0]));
    case 12: return talk_box_l1(talk_fn12(env, talk_unbox_l1(args[0]), talk_unbox_l1(args[1])));
    case 13: return talk_fn13(env, args[0]);
    case 14: return talk_box_l1(talk_fn14(env, args[0], args[1], args[2], args[3]));
    case 15: return talk_fn15(env, args[0], args[1], args[2], args[3]);
    case 16: return talk_fn16(env, talk_unbox_l1(args[0]));
    case 17: return talk_box_l1(talk_fn17(env, talk_unbox_l1(args[0])));
...
int main(int argc, char **argv) {
    ...
    int status = talk_print(talk_fn8(NULL));
    talk_arena_release();
    talk_effects_release();
    return status;
}
