...
- **Structural cleanup planning at the `CheckedMir` seam** (ADR 0030):
  freestanding candidates and flow bookkeeping remain private to MIR
  construction/flow. Candidate claiming attaches symbol cleanup; one exact
  whole-CFG temp ledger completes normal/unwind actions across scaffold joins
  and verifies path balance. Source moves and runtime transfers are separate,
  and all positional/call-temp lowering heuristics are deleted.
...
change can silently degrade to skips or misses. F-A was a historical example;
its temp materialization is fixed, but the verifier still does not analyze
abort-only unwind entries. O7 remains open for named glue-shape
constructors/predicates in `lambda_g/` used by both emitter and verifier,
plus one per-`Op`
ownership-effect table (`expr.rs` doc comments promoted to a spec) that
`eval.rs`, `interp.rs`, and `balance.rs` all cite, with an exhaustiveness
test so a new op cannot be forgotten by one of the four.

### O8. Fuzzer DEFAULT_SKIPS as shadow bug tracker (RESOLVED)

`tests/fuzz.rs::DEFAULT_SKIPS` is empty after the R9/F-B and F-C fixes;
indices 67 and 155 returned to the deterministic corpus. The checked-MIR
cleanup planner also exposed the latent index-554 trailing-block tail-target
mismatch; `TailReturn` now selects a continuation that accepts its checked
value, and the fixed 1,000-program corpus passes with zero failures. There is
no active shadow bug list. If a future failure must be quarantined, make it a named
expected-fail regression that flips red when fixed and derive any temporary
skip array from those names rather than duplicating prose and indices.
...
finishes). The ownership plan's open residual list is now B11, B12, and
F-D. F-B remains the historical runnable repro for the fixed R9 finding;
an earlier draft of this audit counted it separately.

## Suggested order

O6 → O2 → O10 as filler → O7 alongside → O9 when touching the
