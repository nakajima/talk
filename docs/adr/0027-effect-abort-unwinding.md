...
Status: implemented (2026-07-12); previously accepted (2026-07-11) —
shape approved by Pat; mid-finalizer aborts ARE in scope for v1 (the
forbid-effects-in-finalizer-rows fallback in Open Questions is rejected;
flow must model not-yet-freed fields as live owned places at a
finalizer's effectful call)

## Implementation notes (2026-07-12)

Landed as specified, with these resolutions/refinements:

- **Open question 2 (Deinit rows)**: the checker did NOT enforce purity —
...
- **Open question 1 (mid-finalizer aborts)**: falls out of open question
  2 rather than needing new flow modeling. The finalizer thunk's glue
...
- **Open question 3 (evaluator label identity)**: pinned by test 9; one
...
- **Open question 4 (block cache)**: no new key material was needed —
...
- **Known residual — value calls**: calls through function VALUES
  (eta-expanded effectful functions, effectful closures in
  fields/params) suspend but are not capability-passing at the call site
  (the closure captured its capabilities lexically), so they carry no
  unwind entry yet: owned locals live across such a call still leak on
  an abort through it. The MIR builder's `Unwind` candidates already
  cover these sites conservatively (`call_may_suspend`); only the
  lowering attachment (`lower_value_call`, witness calls) is missing.
- The free-balance verifier ignores unwind operands (they are abort-path
  obligations; its v1 models normal paths) and `Op::Abort` bodies skip
  with the same counted reason the bare-value delivery did.
...
- The free-balance verifier (plan Track 6.1) gains paths to check:
  unwind entries are additional per-path balance obligations over the
  same lowered artifact — the verifier needs no special case, which is
  the point of keeping cleanup as ordinary emitted code.
...
- v1 limitations, accepted: abort-during-unwind traps; `@handle` inside a
  nested block remains unsupported (`mir_lowering.rs:1302-1306`),
  independent of this ADR.
- Implementation waits for approval (plan wave 2); the repro corpus below
  lands first as red tests.

## Acceptance tests (the plan's confirmed shapes, both engines, balance asserted)
...
- **Flow's `Unwind` candidates inside finalizer bodies**: a finalizer
  tears an object down field-by-field as sequential statements, not as
  local scope exits. For the mid-finalizer abort (test 5) to free the
  *remaining* fields, flow must model the not-yet-freed fields as live
  owned places at the finalizer's effectful call — `Open`-style
  precision. DECIDED 2026-07-11: this is in scope for v1; the
  forbid-effects-in-finalizer-rows fallback is rejected. Test 5 is a
  v1 acceptance test, not a tracking red.
- **Deinit rows**: the abort-during-unwind argument assumes deinit bodies
  cannot carry user effects. Verify the checker enforces this at the
  `Deinit` conformance; if it doesn't, it must (independently defensible:
  drop glue has no capabilities to pass).
- **Evaluator label identity**: the extent stack keys on `Func` label
  equality across an activation; Eqs. 17-18 freshen dependent functions,
  and LIFO covers capture-free ones, but this is asserted from the
  substitution rules, not yet from a test — test 9 exists to pin it
  before implementation proceeds.
- **`MirCtxKey`/block-cache interaction**: unwind entries are minted per
  suspension site during MIR lowering; the block cache
  (`src/lower/mod.rs:303-315`) must key them like other continuations so
  cached blocks don't share a stale entry across contexts.
