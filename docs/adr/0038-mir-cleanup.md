...
19. **Identity repair deleted (design session 2026-07-23; the rustc/GHC absolute-identity model).** The systemic root was viewer-relative symbol spelling — symbols minted `Current`-tagged and respelled at every import seam, with per-environment module numbering making one module carry several local ids. Both decisions are gone:

    - **Session-wide module numbering**: `ModuleRegistry` (shared by every `ModuleEnvironment` clone in a session — nested package/library environments clone one base) numbers each stable module identity once. Stdlib modules get fixed ids in a reserved `WellKnown` band descending below `Main` (the well-known-symbol pattern); package graphs reserve ids in the session registry before compiling. One module = one id is structural, so the driver's stable-unification loop, the `aliases` plumbing through all three backend entry points, and the `MODULE_ALIASES` thread-local are deleted at the source.
    - **Absolute identity at mint**: every compile stamps its symbols with its real module id — user programs `Main` (the `DriverConfig` default; the old `Current→Main` fixup is gone), core `Core` (already did), stdlib its `WellKnown` id, packages their reserved ids (already did). `ConformanceId`s mint under their declaring module too, so merged catalogs cannot collide and never respell.
    - **Import verbatim**: `Module::import_as`, `TypeCatalog::import_as` (~290 lines of retag), the `Ty`/`Scheme`/`Predicate`/`Row`/`EffectRow` `import_symbols` family, `ConformanceId::import`, `Symbol::current`, and `ModuleEnvironment::import` are deleted. Modules register at their compiled ids (`import_compiled`); phase-0 merges content without respelling.
    - **Backend deletions, verified before deleted**: `canonical` ran one full suite as a `debug_assert`-identity function (zero repairs fired across 1188 tests), then all 58 call sites, `canonical_ty` (9 sites), `canon_rigid`, `resolved()`'s repair line, `closure_effects`' stamp, `ty_mentions_param`'s dual compare, and `demand`'s dual-spelling filter arm were removed textually. Exactly one narrowing remains in `resolved()`: the `ty_has_var` projection guard is now a `debug_assert` (finalized baked types are var-free by construction, pinned by the exported-scheme leak walk) — delete the assert after a quiet cycle.
    - **Residue cleanup (same day)**: the `some!` dummy-`Symbol` discriminators are replaced by a real `SymbolKind` enum (`declare`/`mint`/`mint_pattern` take the kind, `SymbolKind::of` answers kind-vs-symbol checks; the 145-line macro is deleted, and `mint`'s dead `Main`/`Library`/`Builtin` arms with it). Also deleted once their last callers went: `Symbol::import` and the per-id `import()` constructors, `locals_with_stable_ids`, the vestigial `next_conformance_id` merge line, `resolved()`'s var-free `debug_assert` plus `ty_has_var` itself (the exported-scheme leak walk still pins var-freedom at the frontend), and the LSP parse-fallback workspace's `Current` stamp. `external_module_id` — a 30-line duplicate of `module_id` differing only in its `Current → None` arm, the last place `Current` meant anything — is deleted and every caller unified onto `module_id()` (the LSP jump sites simplify further, since their two branches had already converged on the same lookup); the driver's not-`Current` guard went with it, since module ids are opaque now and the invariant it guarded no longer exists. `ModuleId::Current` survives only as "module zero" in isolated resolver/solver/catalog unit tests via the id-macro's `From<u32>` sugar — kept deliberately: it serves ~90 expectation sites, and deleting it would add code, not remove it.
    - **The one remap seam** (rustc's on-disk-`DefId` ↔ session-`CrateNum` discipline): nothing deserializes modules across sessions today, so the seam is a wire-format obligation, not code — if serialized module images ever carry symbols between sessions, the format must include a dependency table (position → `StableModuleId`) and the loader must remap embedded module tags at decode. Do not skip this when packages grow a binary cache.

    Prior art: rustc `DefId`/`CrateNum` per-session with `StableCrateId` for cross-session identity and remap-at-load; GHC `Name`s carrying their defining `Module` absolutely. Net −813 lines including the residue passes.
...
**ADR 0038 is complete** (19 slices, residue included): stage 1, the checked-patterns publication, committed conformance evidence, the callable-inventory publications, and the identity-repair deletion. What remains open is exactly the deferred list above — each entry with its recorded reason and unblocking condition.
