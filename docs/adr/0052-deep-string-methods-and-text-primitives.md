...
The previous `String.find`/`find_from` byte-offset result is removed after
migration rather than retained as the character-shaped interface. New search
...
boundary.
The compatibility **`find`** and **`find_from`** byte-offset methods were
migration-only. They are removed; character-shaped search uses `StringIndex`,
`StringRange`, `Substring`, or `Bool`, and byte search lives on `UTF8View`.
...
- Callable-label identities are used for extension overloads. Character and
  text overloads therefore have distinct labels where ADR 0041 forbids
  type-only overloading.
- Substring search is grapheme-boundary-safe. Matches are leftmost and
  non-overlapping; an empty needle matches every grapheme boundary, including
  the exclusive end.
- `lines()` recognizes CRLF, CR, LF, NEL, LINE SEPARATOR, and PARAGRAPH
  SEPARATOR. `words()` follows the pinned Unicode 17.0 UAX #29 word-break data.
- Whole-string `is_ascii()` and `is_printable()` are true for empty text. The
  other whole-string classification predicates are false for empty text.
- Unicode case, case-folding, property, word-break, and normalization tables
  are generated by `gen_text_unicode` from the vendored Unicode 17.0 files.
  The generator checks normalization and word boundaries against the official
  conformance suites.
- Until static protocol requirements are available, `FromString` dispatches
  through `parse(text, using: exemplar)`; the exemplar selects the result type.
- Static stored properties are not yet a language feature, so predefined
  character sets use static factories such as `CharacterSet.whitespace()`.
- Compatibility byte-offset `String.find` and `find_from` are removed after
  migrating their callers to `UTF8View.find_bytes` and `find_bytes_from`.
- `translating` remains gated exactly as specified in section 4: Core does not
  yet have the required dictionary key protocol.
