...
open residuals live in the findings inventory: R9, B11, B12, and the
fuzzer's F-A/F-C/F-D below)
...
Discovered during wave-2 implementation (pre-existing; unassigned triage):
...
| R9 | ADR 0027 residual: calls through function VALUES (eta-expanded effectful functions, effectful closures in fields/params) suspend but carry no unwind entry — owned locals live across such a call still leak on an abort through it; MIR candidates already cover the sites, only the lowering attachment (`lower_value_call`, witness calls) is missing | src/lower/ value-call lowering |
...
| ID | Finding | Locus |
|----|---------|-------|
| F-A | A `.clone()` result temp used in borrow position (argument to a borrow param, or method receiver) is never dropped: `blen(s.clone())` / `xs.clone().count` leak the refcount bump on BOTH engines, and the free-balance verifier does not flag it. Consume-position (`eat(s.clone())`), let-bound, and statement-position clones balance; non-clone call temps (user methods, `show()`, concat) balance — the hole is specific to the Copy/CheapClone `clone` path | temp classification for clone-call results (typing/flow), plus a verifier blind spot |
| F-B | Runnable repro for R9: an abort through a function-VALUE call (`fn()` inside `run_twice`) skips the unwind for the frames above it — the installing frame's owned `consume` param leaks. The direct-call twin balances (ADR 0027 works there) | src/lower/ value-call lowering (R9) |
| F-C | An aborting perform whose argument is an owned temp (`'oops("burn" + "!")`) leaks that temp on both engines, even with no other owned values live; literal-argument aborts balance | perform-site temp drop on the abort edge (R8-adjacent) |
...
`runtime_moves_enabled` stays a field: it is per-construct suppression
(effect performs) orthogonal to pass phasing. A characterization test
pins top-level errors reporting exactly once under the twice-checked
role convention (`flow_tests.rs::reports_top_level_flow_error_exactly_once`).
No wrong-mode side effects were found; `consumed_temps`/`borrowed_params`
remain deliberately pass-independent (idempotent checker-level facts).
...
binder arms in `arm_release_binders`, `emit_unwind_drop_candidates`,
...
four roots; the failing default-seed indices are skip-listed by name in
`DEFAULT_SKIPS` until the findings are fixed, so the CI run is green and
regression-sensitive everywhere else. Failing repros deliberately NOT
added to tests/programs/ yet — they become corpus entries when their
findings are fixed.
