...
| LIT-01 | Signed 64-bit Int literal range and trapping arithmetic | Required | Integrated `i64` value and `InvalidInt` recovery with boundary diagnostics | Integrated `i64` constant and explicit potentially-trapping scalar operations | Integrated `i64` constant | Integrated boundaries, overflow traps, checked results, and explicit unchecked wrapping | None |
